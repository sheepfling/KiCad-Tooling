...
def main() -> int:
    ...
    parser.add_argument(
        "--project-id", required=True, help="Registered pcb or schematic project ID"
    )
    ...
    parser.add_argument(
        "--cli", default="kicad-cli", help="Exact catalogued local KiCad CLI for --capture"
    )
    parser.add_argument(
        "--runner",
    ...
    try:
        receipt = (
            ...
            if args.capture or args.output is not None
            else None
        )
        if args.capture:
            if receipt is None:
            ...
            summary = (
                args.native_summary
                if args.native_summary.is_absolute()
    ...
    print(
        ...
        if args.format == "json"
        else text_report(report, args.detail)
    )
