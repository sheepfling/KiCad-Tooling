...
def main() -> int:
    ...
    setup.add_argument("--init", action="store_true", help="Create and connect a pending contract; never overwrite existing requirements")
    setup.add_argument("--capture-inputs", action="store_true", help="Write UNREVIEWED design/model hashes under build/; never update approved bindings")
    parser.add_argument("--model", action="append", default=[], help="Repository-relative deck or include for --capture-inputs; repeat for dependencies")
    parser.add_argument("--ngspice-version", help="Engineer-selected version for --init; otherwise UNREVIEWED")
    parser.add_argument("--output", type=Path, help="Fresh analysis/input receipt directory below build/; unavailable with --init")
    parser.add_argument("--native-summary", type=Path, help="Reuse a current source-bound native project summary for analysis")
    parser.add_argument("--runner", choices=("auto", "local", "container"), default="auto", help="KiCad netlist runner; ngspice runs on the host")
    parser.add_argument("--cli", default="kicad-cli", help="Exact local KiCad executable")
    parser.add_argument("--ngspice", default="ngspice", help="Exact contract-pinned host simulator executable")
    parser.add_argument("--format", choices=("text", "json"), default="text")
    parser.add_argument("--detail", choices=("brief", "full"), default="brief", help="Analysis text detail; JSON always includes every result")
    ...
    if (args.init or args.capture_inputs) and (args.native_summary or args.runner != "auto"
                                              or args.cli != "kicad-cli" or args.ngspice != "ngspice"):
        parser.error("Setup does not run native tools; omit --native-summary, --runner, --cli and --ngspice")
    ...
    try:
        if args.init:
            ...
            print(created.model_dump_json(indent=2) if args.format == "json" else summary("Electrical setup", created))
            return 0
        if args.capture_inputs:
            inventory = capture_inputs(args.root, args.project, tuple(args.model), args.output)
            print(inventory.model_dump_json(indent=2) if args.format == "json" else (
        ...
        return 2
    print(report.model_dump_json(indent=2) if args.format == "json" else format_report(report, args.detail))
