...
def executable() -> Path:
    ...
    candidates = [Path(str(installed.locate_file(item))) for item in installed.files or ()
...
def run(arguments: list[str]) -> int:
    ...
    try:
        ...
        print(f"Markdown checker unavailable: {exc}. Reinstall the project tooling extras.",
              file=sys.stderr)
        return 127
