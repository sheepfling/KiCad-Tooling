...
FORBIDDEN_SOURCE_PARTS = frozenset({
    "build", "generated", "schemas", "releases", "release", "restores", "__pycache__",
})
...
def _bounds(offset: int, limit: int, maximum: int) -> None:
    if isinstance(offset, bool) or isinstance(limit, bool) or offset < 0 or not 1 <= limit <= maximum:
...
def list_artifacts(root: Path, directory: str = "build", offset: int = 0,
                   limit: int = 100) -> McpArtifactList:
    ...
    for child in children[offset:offset + limit]:
        ...
        if stat.S_ISDIR(information.st_mode):
            ...
            entries.append(McpArtifactEntry(path=name, kind="file", size_bytes=information.st_size,
                                            sha256=digest))
        else:
            raise ValueError(f"Artifact is not a regular file or directory: {name}")
    next_offset = offset + len(entries)
    more = next_offset < len(children)
    return McpArtifactList(directory=base.relative_to(root.resolve()).as_posix(),
                           entries=tuple(entries), offset=offset, total_entries=len(children),
                           truncated=more, next_offset=next_offset if more else None)


def _content(root: Path, path: Path, offset: int, limit: int,
             permit_text: bool) -> McpFileContent:
    _bounds(offset, limit, MAX_CHUNK)
    relative = path.relative_to(root.resolve()).as_posix()
    size = path.stat().st_size
    if size > MAX_TEXT_BYTES or not permit_text:
        digest, size = _digest(path)
        return McpFileContent(path=relative, sha256=digest, size_bytes=size,
                              content_kind="metadata_only", offset=offset,
    ...
    try:
        ...
        return McpFileContent(path=relative, sha256=digest, size_bytes=len(data),
                              content_kind="binary", offset=offset,
                              note="Binary content is not embedded in MCP responses.")
    next_offset = min(offset + limit, len(text))
    more = next_offset < len(text)
    return McpFileContent(path=relative, sha256=digest, size_bytes=len(data),
                          content_kind="text", text=text[offset:offset + limit], offset=offset,
                          total_characters=len(text), truncated=more,
                          next_offset=next_offset if more else None)


def read_artifact(root: Path, path: str, offset: int = 0,
                  limit: int = MAX_CHUNK) -> McpFileContent:
    """Read a UTF-8 artifact chunk; unsupported/binary files return metadata only."""
    selected = artifact_path(root, path)
    return _content(root, selected, offset, limit, selected.suffix.lower() in ARTIFACT_TEXT_SUFFIXES)
...
def read_project_file(root: Path, project_id: str, path: str, offset: int = 0,
                      limit: int = MAX_CHUNK) -> McpFileContent:
...
def _validate_edit(root: Path, project_id: str, path: Path,
                   text: str) -> Literal["JSON_MODEL", "TEXT_ONLY"]:
    island = _island(root, project_id)
    ...
    if relative == "project.json":
        ...
        for name in (*manifest.source_roots, *manifest.required_inputs,
                     *(item for item in (manifest.mechanical_handoff, manifest.governance_record)
                       if item is not None)):
...
def _prepare_edit(root: Path, project_id: str, path: str, expected_sha256: str,
                  old_text: str, new_text: str) -> _PreparedEdit:
    ...
    if first_match < 0 or text.find(old_text, first_match + 1) >= 0:
        raise ValueError("old_text must match exactly once; include enough unique surrounding context")
...
def preview_project_edit(root: Path, project_id: str, path: str, expected_sha256: str,
                         old_text: str, new_text: str) -> McpEditPreview:
...
def apply_project_edit(root: Path, project_id: str, path: str, expected_sha256: str,
                       old_text: str, new_text: str) -> McpEditResult:
    ...
    try:
        ...
        if (current_bytes != edit.before
                or _fingerprint(information) != _fingerprint(edit.information)):
        ...
        if readback != edit.preview.after_sha256:
            raise ValueError("Source changed after publication; inspect the current file before retrying")
        return McpEditResult(project_id=project_id, path=edit.preview.path,
                             before_sha256=edit.preview.before_sha256,
                             after_sha256=edit.preview.after_sha256, readback_sha256=readback,
