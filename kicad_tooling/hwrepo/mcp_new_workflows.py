...
def _bundle_directory(root: Path, source: CadSourceReport) -> Path:
    ...
    expected = (
        "build",
        "cad-source-cache",
        f"easyeda-{source.bundle.converter_version}",
        source.supplier_id,
    )
    if (
        len(relative.parts) != 6
        or relative.parts[:4] != expected
        or relative.parts[-1] != "bundle"
        or len(relative.parts[4]) != 64
...
def _source_report(
    root: Path, source_report: str, supplier_id: str, expected_mpn: str | None = None
) -> CadSourceReport:
    ...
    if report.supplier_id != supplier_id or (
        expected_mpn is not None and (report.bundle is None or report.bundle.mpn != expected_mpn)
    ):
...
def export_electrical_charts(root: Path, view_id: str, receipt: str) -> ElectricalChartsReport:
    ...
    return electrical_charts.build_charts(
        root, source, fresh_output(root, "electrical-charts", view_id)
    )


def export_electrical_chart_suite(
    root: Path, view_id: str, suite: str
) -> ElectricalChartsSuiteReport:
    source = artifact_file(root, suite)
    if source.suffix != ".json":
        raise ValueError("Select a saved electrical suite JSON")
    return electrical_charts.build_suite(
        root, source, fresh_output(root, "electrical-charts", view_id)
    )
...
def preview_cad_import(
    root: Path, project_id: str, view_id: str, source_report: str
) -> CadImportReport:
    selected_project(root, project_id)
    path = artifact_file(root, source_report)
    if path.name != "cad-source.json":
    ...
    return cad_library.plan(
        root,
        project_id,
        _bundle_directory(root, source),
...
def apply_cad_import(
    ...
    with tempfile.TemporaryDirectory(
        prefix=".reviewed-cad-import-", dir=output.parent
    ) as temporary:
