...
def scan_imports(root: Path, source_directory: Path,
                 toolchain_id: str) -> ImportInventoryReport:
    """Preview each candidate through the normal importer; never copy source."""
    source = source_directory.resolve()
    if not source.is_dir():
        return ImportInventoryReport(source_directory=str(source), status="NEEDS_WORK",
    ...
    for path in sorted(source.rglob("*"), key=lambda item: str(item).casefold()):
        ...
        candidates.append(ImportInventoryCandidate(
    ...
    return ImportInventoryReport(
        ...
        status="PASS" if candidates and not issues and all(candidate.preview.status == "PASS"
                                                           for candidate in candidates) else "NEEDS_WORK",
        candidates=tuple(candidates), skipped_local_state=tuple(skipped), issues=tuple(issues),
    )


def format_import_inventory(report: ImportInventoryReport) -> str:
    lines = [f"Import inventory: {report.status}; {len(report.candidates)} candidate(s)",
    ...
    for candidate in report.candidates:
        ...
        lines.append(f"    {kind}; suggested ID: {candidate.suggested_project_id}; "
    ...
    if report.skipped_local_state:
        lines.append(f"Skipped {len(report.skipped_local_state)} local-state candidate(s); "
                     "see JSON for paths")
