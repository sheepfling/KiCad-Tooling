...
def command_arguments(
    settings: ReleaseExportSettings, base: Path, output: Path, assembly_variant: str | None = None
) -> dict[str, tuple[str, ...]]:
    ...
    commands = {
        "gerbers": (
            ...
            "gerbers",
            "--layers",
            ",".join(settings.gerber_layers),
            "--no-protel-ext",
            "--check-zones",
            *origin,
            *variant,
            "--output",
            str(output / "fabrication"),
            board,
        ),
        "drill": (
            "pcb",
        ...
        "position": (
            ...
            "csv",
            "--units",
            settings.position_units,
            "--exclude-dnp",
            *origin,
            *variant,
            "--output",
            str(output / "assembly/positions.csv"),
            board,
        ),
        "bom": (
            "sch",
            ...
            "bom",
            "--fields",
            "Reference,Value,Footprint,PART_ID,DNP",
            "--labels",
            "Reference,Value,Footprint,PartID,DNP",
            "--exclude-dnp",
            *variant,
            "--output",
            str(output / "assembly/bom.csv"),
            schematic,
        ),
        "schematic_pdf": (
            "sch",
            ...
            *variant,
            "--output",
            str(output / "review/schematic.pdf"),
            schematic,
        ),
        "pcb_pdf": (
            "pcb",
        ...
        "board_stats": (
            ...
            "json",
            "--output",
            str(output / "review/board-stats.json"),
            board,
        ),
    }
    for format_name in settings.supplier_formats:
        if format_name == "odb":
            commands["odb"] = (
                ...
                *variant,
                "--output",
                str(output / "fabrication/board.odb.zip"),
                board,
            )
            ...
            commands["ipc2581"] = (
                ...
                *variant,
                "--output",
                str(output / "fabrication/board.ipc2581.zip"),
                board,
            )
        else:
            commands["ipcd356"] = (
                ...
                "ipcd356",
                "--output",
                str(output / "fabrication/board.d356"),
                board,
            )
    return commands
...
def require_declared_variant(base: Path, name: str | None) -> None:
    ...
    if name not in names:
        raise ValueError(
            f"KiCad assembly variant {name!r} is not declared in {base}; "
            f"available: {names}. Do not export the default population by accident"
        )


def export(
    root: Path, manifest_path: str, output: Path, cli: str, assembly_variant: str | None = None
) -> ReleaseExportReport:
    ...
    commands = {
        name: execute((cli, *arguments), root, output, name)
        for name, arguments in command_arguments(
            settings,
            repo_path(root, config.project),
            output,
            selected_variant,
        ).items()
    }
    ...
    if commands["bom"].returncode == 0:
        try:
            purchasing_bom(
                root, output / "assembly/bom.csv", output / "assembly/purchasing-bom.csv"
            )
        except (OSError, ValueError) as exc:
            issues.append(f"Purchasing BOM: {exc}")
    expected = (
        tuple((output / "fabrication").glob("*.gbr")),
    ...
    passed = (
        not issues
        and all(command.returncode == 0 and command.error is None for command in commands.values())
        and all(
            group and all(path.is_file() and path.stat().st_size for path in group)
    ...
    report = ReleaseExportReport(
        ...
        settings=settings,
        assembly_variant=selected_variant,
...
def review_artifacts_valid(output: Path, settings: ReleaseExportSettings) -> bool:
    try:
        ...
        archives = {"odb": "fabrication/board.odb.zip", "ipc2581": "fabrication/board.ipc2581.zip"}
...
def verify_exports(
    ...
    if (
        ...
        or report.toolchain_id != config.toolchain_id
        or report.settings != settings
        or report.assembly_variant
        != (assembly_variant or (settings.assembly_variant if settings else None))
    ):
    ...
    if set(report.commands) != {
        "version",
        *command_arguments(
            settings,
            repo_path(root, config.project),
            path.parent,
            report.assembly_variant,
        ),
    } or any(
    ...
    if (
        not any(name.endswith(".gbr") for name in report.artifacts_sha256)
        or not any(name.endswith(".drl") for name in report.artifacts_sha256)
        or not set(required_artifacts(settings)) <= report.artifacts_sha256.keys()
        or not review_artifacts_valid(
            path.parent,
            settings,
        )
    ):
