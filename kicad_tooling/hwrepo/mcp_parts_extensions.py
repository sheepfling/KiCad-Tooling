...
def prepare_part_picker(
    ...
    *,
    ...
    if native_summary is None and not allow_checks:
        raise ValueError(
            "Fresh picker capture requires --allow-checks as well as --allow-exports; "
...
def preview_part_selection(
    ...
    if not assignments or any(
        item.part_id not in choices.get(item.reference, ()) for item in assignments
    ):
        raise ValueError(
            "Choose at least one part offered for its reference in the retained picker"
        )
    spec = PartSelectionMap(
        project_id=project_id,
        preconditions=picker.selection_template.preconditions,
