...
def selected_findings(
    root: Path, project_id: str, path: Path, journal: DiagnosticJournal,
    ...
    try:
        ...
        return [finding(
            "BLOCKING", "SELECTED_CONFIG", relative, str(exc),
    ...
    try:
        ...
        findings.append(finding(
            "BLOCKING", "SELECTED_DESIGN", config.project, str(exc),
    ...
    for name in manifest.shared_inputs:
        if not any(name.startswith(f"{source}/") for source in manifest.shared_source_roots):
            findings.append(finding(
                "BLOCKING", "SELECTED_SHARED_INPUT", relative, name,
    ...
    for name in (*config.source_roots, *config.required_inputs):
        try:
            ...
            findings.append(finding(
                "BLOCKING", "SELECTED_PATH", relative, f"{name}: {exc}",
                "Correct the exact portable source path in this board's manifest.",
                DIAGNOSTICS_GUIDE,
            ))
    if any(row.code in {"SELECTED_PATH", "SELECTED_SHARED_ROOT", "SELECTED_SHARED_INPUT"}
           for row in findings):
        return findings
    ...
    try:
        ...
        for name in sorted(expected - observed):
            findings.append(finding(
                "BLOCKING", "SELECTED_INPUT_MISSING", name,
        ...
        for name in sorted(observed - expected):
            findings.append(finding(
                "BLOCKING", "SELECTED_INPUT_UNLISTED", name,
        ...
        findings.append(finding(
            "BLOCKING", "SELECTED_INVENTORY", relative, str(exc),
    ...
    for name in config.required_inputs:
        ...
        if not path.is_file() or not (
            ...
            continue
        try:
            findings.extend(repository_guidance(issue, major) for issue in cad_dependencies(
                root, path, project_dir, major, inventory, roots,
            ))
        except (OSError, UnicodeError, ValueError) as exc:
            findings.append(finding(
                "BLOCKING", "SELECTED_CAD_PARSE", name, str(exc),
...
def rescue_project(root: Path, project_id: str, journal: DiagnosticJournal) -> LocalRescueReport:
    ...
    try:
        ...
        findings = [finding(
            "BLOCKING", "RESCUE_SELECTION", "catalog/projects.json", str(exc),
    ...
    return LocalRescueReport(
        ...
        local_inspection="NEEDS_REPAIR" if any(row.severity == "BLOCKING" for row in findings)
        else "CLEAR",
        findings=tuple(findings), omitted_checks=OMITTED,
        next_command=command, run_directory=str(journal.directory),
    )


def format_rescue(
    result: LocalRescueReport, detail: Literal["brief", "full"] = "brief",
) -> str:
