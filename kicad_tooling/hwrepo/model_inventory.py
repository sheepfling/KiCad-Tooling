...
def _finding(
    code: str, location: str, observed: str, action: str, blocking: bool = False
) -> DiagnosticFinding:
...
def _resolve_model(
    ...
    if not value:
        return ModelAssignment(
            path=value, line=line, resolution="broken", reason="Empty 3D model path"
        )
    ...
    if "\\" in value or value.startswith("/") or PureWindowsPath(value).drive:
        return ModelAssignment(
            path=value, line=line, resolution="broken", reason="Machine-local or backslash path"
        )
    ...
    if (
        ...
        if value.startswith("${" + variables[0] + "}/") and ".." not in value.split("/"):
            return ModelAssignment(
                path=value,
                line=line,
                resolution="toolchain_dependent",
    ...
    if any(variable != "KIPRJMOD" for variable in variables) or "$" in value.replace(
        ...
        return ModelAssignment(
            path=value, line=line, resolution="broken", reason="Undeclared path variable"
        )
    if value.count("${KIPRJMOD}") > 1:
        return ModelAssignment(
            path=value, line=line, resolution="broken", reason="Repeated KIPRJMOD variable"
        )
    ...
    return ModelAssignment(path=value, line=line, resolution="source_present", source_path=relative)
...
def inspect_models(root: Path, config: ProjectConfig) -> ModelInventoryReport:
    ...
    if not board.is_file():
        finding = _finding(
            "MODEL_BOARD_MISSING",
            board_name,
            "PCB source file is missing",
            "Restore the authoritative PCB source, then rerun the inventory.",
            True,
        )
    ...
    try:
        ...
        if (
            len(roots) != 1
            or not _atoms(source, roots[0])
            or _atoms(source, roots[0])[0] != "kicad_pcb"
        ):
    ...
    for node in board_nodes:
        ...
        if missing_reference:
            ...
            findings.append(
                _finding(
                    "MODEL_REFERENCE_UNKNOWN",
                    f"{board_name}:{line(node.start)}",
                    f"Footprint {footprint_id!r} has no readable reference designator",
                    "Open this footprint in KiCad and restore its reference designator.",
                    True,
                )
            )
        ...
        for model_node in model_nodes:
            ...
            embedded_issue = next(
                (
                    ...
                    and "missing embedded model" in issue
        ...
        visible = tuple(
            entry for entry in entries if not entry.hidden and entry.resolution != "broken"
        )
        has_step_model = any(
        ...
        for entry in visible:
            ...
            if (
                ...
                findings.append(
                    _finding(
                        "MODEL_STEP_SUBSTITUTE",
                        f"{board_name}:{entry.line}",
        ...
        status: InventoryStatus = (
            "FAIL"
            if missing_reference or any(entry.resolution == "broken" for entry in entries)
            else "REVIEW"
            if not entries
            or all(entry.hidden for entry in entries)
            or missing_step_substitute
            or idf_only
        ...
        footprints.append(
            FootprintModels(
                ...
                candidate_assets=exact_candidates,
                status=status,
            )
        )
    if not footprints:
        findings.append(
            _finding(
                "MODEL_NO_FOOTPRINTS",
                board_name,
                "Board contains no placed footprints",
