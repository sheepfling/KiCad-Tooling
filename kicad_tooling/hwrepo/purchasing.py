...
def read_components(netlist_path: Path) -> tuple[PurchasingComponent, ...]:
    ...
    for element in sections[0]:
        ...
        components.append(
            PurchasingComponent(
                reference=reference,
                value=_single_text(element, "value"),
                footprint=_single_text(element, "footprint"),
...
def _finding(
    code: str, references: tuple[str, ...], message: str, action: str
) -> PurchasingFinding:
...
def _preference_findings(
    preferences: PurchasingPreferences, parts: dict[str, PartRecord], allowed: set[str]
) -> tuple[PurchasingFinding, ...]:
    findings: list[PurchasingFinding] = []
...
def _component_findings(
    component: PurchasingComponent, parts: dict[str, PartRecord], allowed: set[str]
) -> tuple[PurchasingFinding, ...]:
    findings: list[PurchasingFinding] = []
    ...
    if not component.footprint.strip():
        findings.append(
            _finding(
                "MISSING_FOOTPRINT",
                references,
                "Fitted component has no footprint.",
                "Assign and review its physical footprint in KiCad.",
            )
        )
    if not component.value.strip():
        findings.append(
            _finding(
                "MISSING_VALUE",
                references,
                "Fitted component has no value.",
                "Enter its reviewed value in the schematic.",
            )
        )
...
def plan(
        ...
        if count > 1:
            findings.append(
                _finding(
                    "DUPLICATE_REFERENCE",
                    (reference,),
                    "Component reference appears more than once.",
                    "Repair duplicate schematic references.",
                )
            )
    excluded = tuple(
        component.reference
    ...
    if not fitted:
        findings.append(
            _finding(
                "NO_FITTED_COMPONENTS",
                (),
                "No fitted components are available.",
    ...
    for identifier, group in sorted(groups.items()):
        ...
        lines.append(
            PurchasingLine(
                ...
                required=required,
                spares=spares,
                quantity=required + spares,
                order_number=order_number,
                order_number_kind="DigiKey" if sku is not None else "MPN",
                search_url="https://www.digikey.com/en/products?keywords="
                + quote(
                    order_number if sku is not None else f"{part.manufacturer} {part.mpn}", safe=""
                ),
            )
        )
    ...
    return PurchasingPlan(
        status="NEEDS_PARTS" if findings else "READY_FOR_ORDER_REVIEW",
    ...
    for component in purchase_plan.components:
        ...
        review_rows.append(
            (
                ...
                component.value,
                component.footprint,
                component.part_id or "",
                "",
                "",
                "yes" if component.dnp else "no",
                "yes" if component.exclude_from_bom else "no",
                "",
                "",
                "",
                "",
