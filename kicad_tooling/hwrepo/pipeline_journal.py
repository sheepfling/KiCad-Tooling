...
class PipelineJournal:
    def __init__(
        self, output: Path | None, scope: str, projects: tuple[str, ...], workers: int
    ) -> None:
        self.output = output
    ...
    def _event(
        self, name: str, status: str, elapsed: float | None = None, error: str | None = None
    ) -> None:
    ...
    def stage(self, name: str, action: Callable[[], _Result]) -> _Result:
        ...
        try:
            ...
            self._event(name, "ERROR", time.monotonic() - started, f"{type(exc).__name__}: {exc}")
            raise
        ...
        state = (
            ...
            if isinstance(result, CommandEvidence)
            else getattr(result, "status", "DONE")
        )
