...
def _catalog(root: Path) -> PartsCatalog:
    ...
    if len({part.id for part in catalog.parts}) != len(catalog.parts):
        raise ValueError(
            "Parts catalog repeats PART_IDs; repair duplicate records before selecting"
        )
    return catalog
...
def _model(root: Path, project_id: str, part: PartRecord) -> tuple[Path, str, str, str]:
    ...
    return _model_path(
        ...
        repo_path(root, project.project).parent,
        manifest,
        part.cad.model,
    )
...
def _verify_snapshot(root: Path, project_id: str, expected: Mapping[str, str | None]) -> None:
    ...
    if current != dict(expected):
        changed = sorted(
            ...
            or current.get(name) != expected.get(name)
        )
        raise ValueError(
            "Source or selection inputs changed, or preconditions are incomplete: "
            + ", ".join(changed[:8])
            + ". Create and review a fresh selection."
        )


def _choices(
    root: Path, project_id: str, components: tuple[PartCadComponent, ...], catalog: PartsCatalog
) -> tuple[tuple[PartPickerItem, ...], tuple[PartRecord, ...]]:
    ...
    for component in components:
        ...
        if component.dnp or component.exclude_from_bom:
            ...
            for part in sorted(catalog.parts, key=lambda item: item.id):
                if part.cad is None or (part.cad.symbol_id, part.cad.value) != (
                    component.symbol_id,
                    component.value,
                ):
                    continue
            ...
            if not choices:
                issues.append(
                    "Add an approved real catalog part with a reviewed CAD binding for "
        ...
        items.append(
            PartPickerItem(component=component, choice_ids=tuple(choices), issues=tuple(issues))
        )
...
def create_picker(
    ...
    try:
        ...
        if evidence.status != "READY_FOR_REVIEW":
            raise ValueError(
                "; ".join(evidence.issues) + ". Declare reviewed model assets in "
        ...
        if actual != observed:
            raise ValueError(
                "Native component inventory differs from the editable schematic; "
        ...
        (output / "selection-draft.json").write_text(
            draft.model_dump_json(indent=2) + "\n", encoding="utf-8"
        )
        return PartPickerReport(
            status="READY" if choices else "NEEDS_CATALOG",
            project_id=project_id,
            items=items,
            choices=choices,
            selection_template=draft,
...
def _planned_edits(
    root: Path, project_id: str, spec: PartSelectionMap
) -> tuple[tuple[PartSourceEdit, ...], tuple[str, ...]]:
    if not spec.assignments:
        raise ValueError(
            "Choose at least one compatible catalog part in the picker before previewing"
        )
    project_context(
        root, project_id
    )  # Never bypass declared native-source inventory with an edited map.
    catalog = _catalog(root)
    records = {part.id: part for part in catalog.parts}
    components = {
        component.reference: component for component in read_cad_components(root, project_id)
    }
    ...
    for assignment in spec.assignments:
        ...
        if component is None or component.dnp or component.exclude_from_bom:
            raise ValueError(
                f"{assignment.reference}: selection is not a fitted schematic component"
            )
        if part is None:
        ...
        if part.cad is None or (part.cad.symbol_id, part.cad.value) != (
            ...
            raise ValueError(
                f"{assignment.reference}: selected part must preserve the exact symbol and electrical value"
            )
    ...
    _require_shared_consumer_inventory(
        root, load_registry(root), project.config, manifest, additions["shared_inputs"]
    )
    ...
    replaced_ids = {
        components[reference].part_id
        for reference, part in selected.items()
        if components[reference].part_id is not None and components[reference].part_id != part.id
    }
    remaining_ids = {
        selected[reference].id if reference in selected else component.part_id
        for reference, component in components.items()
    }
    retired_ids = tuple(
        sorted(identifier for identifier in replaced_ids - remaining_ids if identifier is not None)
    )
    manifest_before = _text(manifest_path)
    manifest_after = update_project_manifest_parts(
        manifest_before, tuple(part.id for part in selected.values()), additions, retired_ids
    )
    if manifest_before != manifest_after:
        edits.append(
            PartSourceEdit(path=project.config, before=manifest_before, after=manifest_after)
        )
    prefs_path = preferences_path(root, project_id, None)
    prefs_before = _text(prefs_path) if prefs_path.exists() else None
    prefs = (
        read_model(prefs_path, PurchasingPreferences)
        if prefs_before is not None
        else PurchasingPreferences()
    )
    overrides = {
        identifier: sku
        for identifier, sku in prefs.digikey_skus.items()
    ...
    for part in selected.values():
        if part.cad is not None and part.cad.digikey_sku is not None:
            ...
            if existing is not None and existing != part.cad.digikey_sku:
                raise ValueError(
                    f"{part.id}: saved DigiKey SKU conflicts with the reviewed CAD catalog binding; review both records"
                )
            overrides[part.id] = part.cad.digikey_sku
    prefs_after = (
        PurchasingPreferences(
            boards=prefs.boards,
            spare_percent=prefs.spare_percent,
            spare_minimum=prefs.spare_minimum,
            digikey_skus=overrides,
    ...
    if prefs_before != prefs_after:
        edits.append(
            PartSourceEdit(
                path=prefs_path.relative_to(root).as_posix(), before=prefs_before, after=prefs_after
            )
        )
...
def _contract_guidance(root: Path, project_id: str, spec: PartSelectionMap) -> tuple[str, ...]:
    ...
    for assignment in spec.assignments:
        ...
        if part.cad is not None and (
            expected is None
            or (expected.part_id, expected.value, expected.footprint)
    ...
    return (
        (
            f"Review independent engineering requirements in {contract} for {', '.join(sorted(different))}: "
...
def _apply_edits(
    ...
    preconditions: Mapping[str, str | None],
) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix=".part-selection-", dir=root / "build") as temporary:
        ...
        try:
            ...
            for index, edit in enumerate(reversed(applied)):
                ...
                try:
                    if not path.is_file() or digest(path) != _sha(edit.after):
                        raise ValueError(
                            f"{edit.path}: changed externally; retained for manual recovery"
                        )
            ...
            if rollback_errors:
                raise ValueError(
                    f"Selection failed: {error}. Rollback needs review: "
...
def selection(
    root: Path, project_id: str, map_path: Path, output: Path, apply: bool = False
) -> PartSelectionReport:
    root = root.resolve()
    edits: tuple[PartSourceEdit, ...] = ()
    pending: tuple[str, ...] = ()
    try:
        ...
        if spec.locked and dict(spec.after_hashes) != after_hashes:
            raise ValueError(
                "Locked selection differs from recalculated edits; preview a fresh draft"
            )
        ...
        diffs = "".join(
            "".join(
                difflib.unified_diff(
                    (edit.before or "").splitlines(keepends=True),
        ...
        commands = (
            (
                (f"python -B -m kicad_tooling.parts --project {project_id} --sync-models",)
                if pending
                else (
        ...
        issues = (
            (
                "Update PCB from Schematic (F8) in KiCad, save, then preview --sync-models for: "
                + ", ".join(pending),
            )
            if pending
            else ()
        ) + contract_guidance
        return PartSelectionReport(
            status=status,
            project_id=project_id,
            edits=edits,
...
def resume_selection(root: Path, project_id: str, output: Path) -> PartSelectionReport:
    ...
    try:
        ...
        assignments = tuple(
            PartSelectionAssignment(reference=component.reference, part_id=component.part_id)
            for component in read_cad_components(root, project_id)
            if not component.dnp
            and not component.exclude_from_bom
            and component.part_id is not None
            and component.part_id in parts
            and parts[component.part_id].cad is not None
        )
        if not assignments:
            raise ValueError(
                "No fitted schematic PART_ID has a reviewed CAD binding; choose parts in --picker first"
            )
        spec = PartSelectionMap(
            project_id=project_id, preconditions=before, assignments=assignments
        )
        ...
        return PartSelectionReport(
            status="BLOCKED", project_id=project_id, issues=(str(error),), receipt_dir=str(output)
        )
