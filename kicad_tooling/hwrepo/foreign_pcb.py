...
def render_text(report: ForeignPcbReport) -> str:
    lines = [
        f"Foreign PCB conversion: {report.status}",
    ...
    if report.native_summary is not None:
        lines.append(
            f"KiCad report: {report.native_summary.mapped_layers} mapped layers, "
    ...
    try:
        with journal.stage("project-selection"):
            if re.fullmatch(r"[A-Za-z0-9][A-Za-z0-9_.-]*", project_id) is None:
                raise ValueError(
                    "Project ID must use letters, digits, periods, underscores or hyphens"
                )
            if re.fullmatch(r"[A-Za-z0-9][A-Za-z0-9_.-]*", toolchain_id) is None:
                raise ValueError(
                    "Toolchain ID must use letters, digits, periods, underscores or hyphens"
                )
        ...
        with journal.stage("source-and-toolchain"):
            if source.is_symlink() or not source.is_file() or source.suffix == ".kicad_pcb":
                raise ValueError(
                    "Select one existing non-KiCad PCB file (not a symlink or native board)"
                )
        ...
        with journal.stage("runner-readiness"):
            ...
            if ready.status != "PASS":
                raise ValueError(
                    "Exact KiCad runner unavailable; inspect doctor.json and run "
        ...
        if selected == "local":
            ...
            convert_argv = (
                ...
                *mounts,
                ...
                input_format,
                "--output",
                f"/output/stage/{board.name}",
                "--report-format",
                "json",
                "--report-file",
                "/output/kicad-import-report.json",
                user_source,
            )
        with journal.stage("version"):
            ...
            if (
                commands["version"].returncode != 0
                or commands["version"].stdout.strip() != record.kicad_version
            ):
                raise ValueError(
                    f"Runner must be exact KiCad {record.kicad_version}; inspect version-command.json"
                )
        with journal.stage("conversion"):
            ...
            if native_summary.errors:
                raise ValueError(
                    f"KiCad import report contains {len(native_summary.errors)} errors; "
        ...
        next_command = (
            "python -B -m kicad_tooling.template import-project --source "
            f"{shlex.quote(str(native_project))} --project-id {shlex.quote(project_id)} "
            f"--toolchain {shlex.quote(toolchain_id)} --format text"
        )
        actions = (
            (
                "Open stage/*.kicad_pcb with the exact KiCad version and inspect layer mapping, "
        ...
        actions = (
            (
                "Inspect events.log, doctor.json, convert-command.json and the raw KiCad report "
