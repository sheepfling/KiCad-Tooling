...
def render_html(report: PurchasingReport) -> str:
    ...
    status = {
        "READY_FOR_ORDER_REVIEW": "Ready for supplier review",
        "NEEDS_PARTS": "Part details need attention",
        "BLOCKED": "Could not read the parts",
    }
    sections: list[str] = []
    ...
    if report.plan is None:
        sections.append(
            ...
            + "".join(f'<p class="notice">{escape(message)}</p>' for message in report.issues)
            + "".join(f"<p>{escape(action)}</p>" for action in report.next_actions)
            + "</section>"
        )
    else:
        plan = report.plan
...
Use an assembly multiplier of 1 and no extra attrition; these quantities already include spares.</p></div>
        ...
        manifest = next(
            (
                name
                for name in report.input_hashes
        ...
        rerun = (
            f"python -B -m kicad_tooling.parts --project {report.project_id} --boards {prefs.boards} "
            f"--spare-percent {prefs.spare_percent} --spare-minimum {prefs.spare_minimum}"
        )
        initialize = (
            f"python -B -m kicad_tooling.parts --project {report.project_id} "
            f"--init-preferences {prefs_path} --boards {prefs.boards} "
            f"--spare-percent {prefs.spare_percent} --spare-minimum {prefs.spare_minimum}"
        )
        sections.append(
            "<section><h2>Adjust the build</h2><p>Change the numbers below and run "
            "the command from your repository. Each run makes a fresh review.</p><p><code>"
            + escape(rerun)
            + "</code></p><details><summary>Save defaults for next time</summary>"
            "<p>Create this file once, then edit its quantities or reviewed DigiKey SKUs. "
            "Future runs load it automatically.</p><p><code>"
            + escape(initialize)
        ...
        if ready:
            downloads = (
                '<a class="button" href="digikey.csv" download>Download DigiKey list</a>'
                + downloads
            )
            guidance = (
                "Map Part Number, Quantity and Customer Reference when uploading. "
                "Confirm each matched product before adding it to your cart."
            )
        else:
            guidance = (
                "Resolve the items below and rerun the command to unlock the DigiKey list. "
                "The review BOM helps you fill in the missing details."
            )
        sections.append(
            f"<section><h2>Order files</h2><p>{guidance}</p>{downloads}"
        ...
        for component in plan.components:
            ...
            line = next(
                (item for item in plan.lines if component.reference in item.references), None
            )
            detail = (
                "Excluded from purchasing"
                if excluded
                else " / ".join(f.message for f in problems)
                if problems
                else "Resolve catalog / preferences above"
                if global_blockers
            ...
            if line is not None:
                ...
                part += (
                    f'<a href="{escape(line.search_url, quote=True)}" target="_blank" '
                    'rel="noopener noreferrer">Find in DigiKey</a>'
                )
            rows.append(
                f'<tr data-issue="{"yes" if problems or (global_blockers and not excluded) else "no"}"><td><strong>'
        ...
        sections.append(
            """<section><h2>Component checklist</h2><div class="toolbar">
...
<th>Catalog part</th><th>Next step</th></tr></thead><tbody>"""
            + "".join(rows)
            + "</tbody></table></div></section>"
        )
        if plan.lines:
            quantities = "".join(
                f"<tr><td>{escape(line.part_id)}<small>{escape(', '.join(line.references))}"
                f"</small></td><td>{line.per_board}</td><td>{line.required}</td>"
                f"<td>{line.spares}</td><td><strong>{line.quantity}</strong></td></tr>"
                for line in plan.lines
            )
            sections.append(
                '<section><h2>Quantity worksheet</h2><p class="muted">'
    ...
    evidence = (
        f"<details><summary>Receipt details</summary><p>Saved design files: "
