...
def _page(project_id: str, title: str, body: str, script: str = "") -> str:
    return (
        ...
        + body
        + "<footer>Reviewed catalog bindings preserve the existing symbol and value. "
...
def render_picker(report: PartPickerReport) -> str:
    ...
    content = [
        (
            '<p class="subtitle">Choose the physical part, footprint, 3D body and supplier number '
    ...
    if report.issues:
        content.append(
            '<section class="notice"><h2>Before you choose</h2>'
            + "".join(f"<p>{escape(issue)}</p>" for issue in report.issues)
            + "</section>"
        )
    if report.evidence is not None and report.evidence.native_status == "FAIL":
        content.append(
            '<section class="notice"><h2>Native validation needs attention</h2>'
    ...
    if report.status == "NEEDS_CATALOG":
        content.append(
            "<section><h2>Add reviewed parts to unlock the picker</h2><p>"
    ...
    if selectable:
        content.append(
            '<form id="picker"><div class="toolbar"><input id="search" type="search" '
    ...
    for index, item in enumerate(report.items):
        ...
        content.append(
            '<section class="component"><div class="component-head"><div>'
        ...
        if selectable and item.choice_ids:
            content.append(
                f'<label for="part-{index}">Choose a reviewed part for '
            ...
            for part_id in item.choice_ids:
                part = parts[part_id]
                content.append(
                    f'<option value="{escape(part.id, quote=True)}">'
            ...
            for part_id in item.choice_ids:
                ...
                if cad is None:
                    continue
                details = (
                    ("Catalog identity", part.id),
                    ("Manufacturer", part.manufacturer),
                    ("Part number", part.mpn),
                    ("Footprint", cad.footprint),
                    ("3D source model", cad.model),
                    ("DigiKey number", cad.digikey_sku or "Use manufacturer part number"),
                )
                content.append(
                    f'<div class="choice" data-part-id="{escape(part.id, quote=True)}" hidden>'
                    "<h3>This choice will use</h3><dl>"
                    + "".join(
    ...
    if selectable:
        ...
        content.append(
            '<div class="selection-bar"><p id="selection-count" aria-live="polite">'
        ...
        content.append(
            '<section><h2>Review before applying</h2><p id="download-note" hidden>'
    ...
    content.append(
        "<section><h2>What happens to the 3D board?</h2><p>"
...
def render_selection(report: PartSelectionReport) -> str:
    ...
    if report.pending_references:
        body.append(
            "<section><h2>Continue in KiCad</h2><p>Use Update PCB from Schematic (F8), "
            ...
            + escape(", ".join(report.pending_references))
            + "</strong></p></section>"
        )
    if report.status == "PLAN":
        body.append(
            '<p class="subtitle">These are the proposed source changes. '
            "Review them before running the apply command below.</p>"
        )
    for edit in report.edits:
        diff = difflib.unified_diff(
            (edit.before or "").splitlines(),
    ...
    if report.next_commands:
        body.append(
            ...
            + "".join(f"<pre>{escape(command)}</pre>" for command in report.next_commands)
            + "</section>"
        )
...
def picker_text(report: PartPickerReport) -> str:
    ...
    lines.extend(
        (
            f"Open: {report.receipt_dir}/index.html",
