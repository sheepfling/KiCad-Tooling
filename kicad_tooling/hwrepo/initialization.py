...
def initialize(root: Path, project_id: str) -> TemplateInitReport:
    ...
    try:
        if layout(root) != RepositoryLayout():
            raise ValueError(
                "Template init requires the default layout; initialize before adapting paths"
            )
        contract = read_model(root / "templates/template-contract.json", TemplateContract)
        adoption = TemplateAdoptionRecord(
            template_version=contract.template_version, project_id=project_id, status="initialized"
        )
        ...
        return TemplateInitReport(
            status="FAIL",
            project_id=project_id,
            issues=(PolicyIssue(code="TEMPLATE_INIT", location="repository", message=str(exc)),),
        )
