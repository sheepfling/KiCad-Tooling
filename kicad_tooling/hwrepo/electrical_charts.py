...
def source_receipt(root: Path, path: Path) -> Path:
    ...
    try:
        ...
        raise ValueError(
            "Electrical charts require a receipt under this repository's build/"
        ) from exc
...
def waveform_csv(path: Path, data: WaveformData) -> None:
    ...
    with path.open("w", newline="", encoding="utf-8") as stream:
        ...
        for series in data.series:
            header.extend(
                (f"{series.name}.real [{series.unit}]", f"{series.name}.imag [{series.unit}]")
            )
        ...
        for index, value in enumerate(data.axis):
            ...
            for series in data.series:
                row.extend(
                    (
                        format(series.values[index].real, ".17g"),
...
def grounding_csv(
    path: Path, contract: GroundingAnalysis, observed: ContractCoachReport | None
) -> None:
    ...
    with path.open("w", newline="", encoding="utf-8") as stream:
        ...
        for domain in contract.domains:
            for pin in domain.pins:
                ...
                status = (
                    "MATCH"
                    if actual == domain.net
                    else ("MISSING_OR_DIFFERENT" if observed is not None else "UNVERIFIED")
                )
        ...
        for rail in contract.rails:
            writer.writerow(
                (
                    ...
                    rail.continuous_limit_a,
                    rail.peak_limit_a,
                    rail.peak_duration_limit_s,
                    max(load.startup_s for load in rail.loads),
                    rail.basis,
                )
            )
...
def build_charts(root: Path, receipt: Path, output: Path | None = None) -> ElectricalChartsReport:
    ...
    directory = receipt_directory(
        root,
        report.project_id,
        output or (Path("build/electrical-charts") / f"{report.project_id}-{uuid4().hex[:12]}"),
    )
    ...
    if isinstance(contract.grounding, GroundingAnalysis):
        ...
        if "netlist-evidence.json" in report.artifacts_sha256:
            observed = read_model(
                checked_artifact(source, report, "netlist-evidence.json"), ContractCoachReport
            )
    ...
    for case in cases:
        ...
        if raw_name not in report.artifacts_sha256 and not (source / raw_name).exists():
            results.append(
                ElectricalChartCase(
                    id=case.id,
                    status="FAIL" if report.status == "PASS" else "SKIPPED",
        ...
        try:
            ...
            csv_name, png_name, svg_name = (
                f"{case.id}.{extension}" for extension in ("csv", "png", "svg")
            )
            ...
            results.append(
                ElectricalChartCase(
                    id=case.id,
                    status="PASS",
                    samples=len(data.axis),
                    waveform_sha256=digest(raw),
                    csv=csv_name,
                    png=png_name,
                    svg=svg_name,
    ...
    output_report = ElectricalChartsReport(
        ...
        next_actions=(
            "Review charts with the declared acceptance limits and the original analysis receipt.",
...
def build_suite(root: Path, suite: Path, output: Path | None = None) -> ElectricalChartsSuiteReport:
    ...
    directory = receipt_directory(
        root, "suite", output or (Path("build/electrical-charts") / f"suite-{uuid4().hex[:12]}")
    )
    ...
    status = (
        ...
        if any(item.status == "PARTIAL" for item in reports)
        else "PASS"
    )
    result = ElectricalChartsSuiteReport(
        status=status, run_directory=str(directory), reports=reports
    )
