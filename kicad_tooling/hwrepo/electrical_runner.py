...
def analyze(root: Path, project_id: str, output: Path | None = None,
        ...
        if contract is None:
            ...
            if isinstance(contract.grounding, GroundingAnalysis):
                if native_summary is None:
                    ...
                    observed = capture(root, project_id, netlist_output, runner or AutoNetlistRunner(cli))
                else:
                ...
                if observed.observed is None:
                    checks.append(ElectricalCheck(id="grounding", status="FAIL",
            ...
            if cases:
                ...
                try:
                    commands["version"] = simulator_version(output, executable, contract.ngspice_version)
                except ValueError as exc:
                    checks.extend(ElectricalCheck(id=case.id, status="NOT_RUN", detail=str(exc)) for case in cases)
                else:
                    for case in cases:
                        try:
