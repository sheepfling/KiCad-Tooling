...
def git_metadata_mounts(root: Path) -> tuple[str, ...]:
    ...
    try:
        ...
        raise ValueError(
            "Cannot resolve linked-worktree Git metadata for the native container"
        ) from exc
    ...
    if not git_dir.is_relative_to(common_dir):
        raise ValueError(
            "Native containers require the Git directory to be inside its common directory"
        )
    ...
    return (
        "--mount",
        f"type=bind,source={source},target={CONTAINER_COMMON},readonly",
        "-e",
        f"GIT_DIR={container_git}",
        "-e",
        f"GIT_COMMON_DIR={CONTAINER_COMMON}",
        "-e",
        "GIT_WORK_TREE=/work",
        "-e",
