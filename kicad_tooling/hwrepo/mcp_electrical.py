...
def capture_electrical_inputs(
    root: Path, project_id: str, view_id: str, models: tuple[str, ...] = ()
) -> ElectricalInputInventory:
...
def analyze_electrical(
    ...
    return electrical_runner.analyze(
        root, project_id, output, summary, cli="kicad-cli", ngspice="ngspice", runner=native_runner
    )
...
def check_electrical_scope(
    ...
    return electrical_runner.analyze_scope(
        root,
        ProjectSelector(
