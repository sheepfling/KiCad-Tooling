...
def main() -> int:
    ...
    parser.add_argument(
        "--project", required=True, help="One registered PCB or PCB-only project ID"
    )
    parser.add_argument(
        "--check-models", action="store_true", help="Inspect model coverage without KiCad"
    )
    parser.add_argument(
        "--map-models",
        type=Path,
    ...
    parser.add_argument(
        "--cli", default="kicad-cli", help="KiCad CLI path for an exact local runner"
    )
    parser.add_argument(
        "--assembly-variant", help="Declared KiCad component population for 3D outputs"
    )
    parser.add_argument(
        "--view",
        dest="views",
        action="append",
        choices=THREE_D_VIEWS,
    ...
    if sum((args.map_models is not None, args.init_model_map is not None, args.check_models)) > 1:
    ...
    if args.check_models and args.runner != "auto":
        parser.error("--runner is used only when generating 3D exports")
    if args.assembly_variant and (
        args.map_models is not None or args.init_model_map is not None or args.check_models
    ):
    ...
    try:
        if args.map_models is not None:
            mapped = populate_models(
                args.root, args.project, args.map_models, apply=args.apply, output=args.output
            )
            print(
                mapped.model_dump_json(indent=2)
                if args.format == "json"
                else render_population_text(mapped)
            )
            return (
                0
                if mapped.status in {"DRAFT", "PLAN", "APPLIED"}
                else (2 if mapped.status == "ERROR" else 1)
            )
        if args.init_model_map is not None:
            mapped = init_model_map(
                args.root, args.project, args.init_model_map, output=args.output
            )
            print(
                mapped.model_dump_json(indent=2)
                if args.format == "json"
