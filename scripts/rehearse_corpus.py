...
NOT_EXERCISED = (
    "transient_and_frequency_simulations",
    "reviewed_part_selection",
    "cad_download_and_import",
    "release_revision_package_and_restore",
)
...
def write_report(output: Path, summary: dict[str, Any]) -> None:
    ...
    lines += [
        "",
        "Grounding and static budgets are checked only when authored requirements exist.",
    ...
    for result in summary["results"]:
        ...
        lines.append(
            f"| [{fixture['id']}]({fixture['id']}/result.json) | {repository} | {states} |"
        )
...
def verify_group(corpus: Path, group: dict[str, Any]) -> list[str]:
    ...
    payload = (
        inventory["files"] if group["source_kind"] == "generated_actions_artifact" else inventory
    )
    ...
    for base, dirs, files in os.walk(directory, followlinks=False):
        ...
        actual.update(
            (Path(base) / name).relative_to(directory).as_posix() for name in [*files, *links]
        )
...
def run(output: Path, name: str, root: Path, *args: str, timeout: int = 300) -> dict[str, Any]:
    command = [
        sys.executable,
        "-I",
        "-B",
...
def execute_one(
            ...
            if imported["returncode"] == 0:
                ...
                if native:
                    stages = [
                        ...
                        ("parts", ("parts", "--project", fixture_id, "--runner", "container")),
                    ]
                    manifest = json.loads((folder.parent / "project.json").read_text())
                    if manifest.get("kind") in {"pcb", "pcb_only"}:
                        stages.append(
                            ("3d", ("visualize", "--project", fixture_id, "--runner", "container"))
                        )
                    for name, arguments in stages:
                        result["stages"].append(
                            run(receipt, name, project, *arguments, timeout=1800)
                        )
        result["status"] = "RECORDED"
        if (
            any(row["parse_error"] or row["returncode"] not in (0, 1) for row in result["stages"])
            or result.get("copied_bytes_match") is False
        ):
    ...
    print(
        f"rehearsal: {fixture_id} {result['status']} "
        + ", ".join(f"{r['stage']}={r['status']}" for r in result["stages"]),
        flush=True,
    )
    return result
...
def main() -> int:
    ...
    fixtures = [
        row
        for row in queue["fixtures"]
        if row["ready"] and (not args.fixture or row["id"] in args.fixture)
    ]
    ...
    save(
        output / "identity.json",
        {
            ...
            "queue_sha256": digest(corpus / "queue.json"),
            ...
            "catalog_counts": queue.get("counts", {}),
        },
    )
    ...
    def integrity() -> dict[str, list[str]]:
        with ThreadPoolExecutor(max_workers=args.jobs) as pool:
            return dict(
                zip(
                    groups,
                    pool.map(lambda group: verify_group(corpus, group), groups.values()),
                    strict=True,
                )
            )
    ...
    initialized = run(
        output, "initialize", baseline, "template", "init", "--project-id", "research-rehearsal"
    )
    ...
    status = (
        ...
        if (
            ...
            and not any(after.values())
            and all(r["status"] == "RECORDED" for r in results)
        )
        else "INCOMPLETE"
    )
    summary = {
        ...
        "selected": len(fixtures),
        "build_authorized": False,
        "results": sorted(results, key=lambda row: row["fixture"]["id"]),
