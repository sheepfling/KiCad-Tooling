...
def source_files(root: Path) -> dict[str, str]:
    ...
    for directory, children, files in os.walk(root, followlinks=False):
        ...
        children[:] = sorted(
            ...
            if name not in OMIT and not ephemeral((base / name).relative_to(root).as_posix())
        )
...
class Playtest:
    ...
    def run(
        ...
        *,
        ...
        self.record(
            ...
            expected_returncode=expected_exit,
    ...
    async def mcp(self, root: Path, toolchain_id: str) -> None:
        ...
        for writable in (False, True):
            ...
            parameters = StdioServerParameters(
                command=sys.executable, args=arguments, cwd=self.caller, env=self.environment
            )
            with (self.output / f"{label}.stderr.log").open("w", encoding="utf-8") as errors:
                async with stdio_client(parameters, errlog=errors) as (read, write):
                    async with ClientSession(read, write, read_timeout_seconds=120) as client:
                        ...
                        async def call(
                            name: str, arguments: dict[str, Any], stage: str, expected_status: str
                        ) -> dict[str, Any]:
                        ...
                        self.record(
                            f"{label}-project-identity",
                            "controller" in {item["id"] for item in inventory.get("projects", [])},
                        )
                        document = await client.call_tool("read_document", {"name": "start-here"})
                        contents = "\n".join(
                            item.text for item in document.content if isinstance(item, TextContent)
                        )
                        ...
                        if not writable:
                            ...
                            self.save(
                                "mcp-default-denied-check.json", denied.model_dump(mode="json")
                            )
                        ...
                        self.record(
                            "mcp-actionable-diagnosis",
                            bool(diagnosis.get("findings"))
                            and any(
                                item.get("severity") == "BLOCKING" and item.get("action")
                        ...
                        await call(
                            "check_project",
                            {"project_id": "mcp-first"},
                            "mcp-check-incomplete",
                            "FAIL",
                        )
                        await call(
                            "check_project",
                            {"project_id": "controller"},
                            "mcp-check-controller",
                            "PASS",
                        )
    ...
    def execute(self) -> None:
        ...
        adoption = self.cli(
            "adopt", adopted, "template", "adopt", "--project-id", "playtest-hardware", timeout=600
        )
        ...
        self.record(
            "import-preview-readonly", not (adopted / "projects/imported-controller").exists()
        )
        ...
        self.record(
            "import-byte-identity",
            bool(copied)
            and copied == preview.get("copied_sha256")
            and all(
                hashlib.sha256((directory / "kicad" / name).read_bytes()).hexdigest() == digest
        ...
        self.record(
            "import-independent-requirements",
            imported.get("review_required") is True
            and validation.get("components") == {}
            and validation.get("nets") == {},
            review_state="UNREVIEWED",
            electrical_coverage=False,
            build_authorized=False,
        )
        self.cli(
            "import-diagnosis",
        ...
        if self.native:
            ...
            self.record(
                "capture-preserves-review-boundary",
                observed.get("review_state") == "UNREVIEWED"
        ...
        self.record(
            "original-native-source-preserved",
            original_native == source_files(self.template / Path(controller["project"]).parent),
        )
...
def main() -> int:
    ...
    requested = args.output or ROOT / "build/playtests" / datetime.now(UTC).strftime(
        "run-%Y%m%dT%H%M%S%fZ"
    )
    output = requested.absolute()
    if output.resolve() != output or not any(
        output.is_relative_to(base / "build") and output != base / "build"
