...
def stage(
    ...
    *,
    ...
    print(f"tooling-ci: {name} {status.lower()} ({entry['elapsed_seconds']}s)", flush=True)
...
def build_distributions(output: Path, build_python: str) -> tuple[Path, str]:
    ...
    stage(
        "wheel",
        (build_python, "-m", "build", "--wheel", "--no-isolation", "--outdir", str(wheels), "."),
        output,
        cwd=ROOT,
    )
    ...
    stage(
        "sdist",
        (build_python, "-m", "build", "--sdist", "--no-isolation", "--outdir", str(sdists), "."),
        output,
        cwd=ROOT,
    )
    ...
    with tempfile.TemporaryDirectory(prefix="kicad-tooling-sdist-") as temporary:
        ...
        if any(source.rglob(".git")) or any(
            (parent / ".git").exists() for parent in source.parents
        ):
    ...
    (output / "packaging.json").write_text(
        json.dumps(
            {
                "version": version,
                "source_distribution_version": rebuilt_version,
                "package_files": len(payload),
                "source_distribution_contains_git": False,
                "status": "PASS",
            },
            indent=2,
        )
...
def adapted_project_fixture(project: Path, destination: Path) -> Path:
    ...
    shutil.copytree(
        ...
        ignore=shutil.ignore_patterns(
            "build", ".git", ".venv", "__pycache__", ".pytest_cache", ".ruff_cache"
        ),
    )
    ...
    moves = (
        ("examples/projects", "engineering/reference"),
        ("projects", "designs"),
        ("catalog", "policy"),
    )
    ...
    discovery["catalogs"] = {key: relocated(value) for key, value in discovery["catalogs"].items()}
...
def verify_adapted_project(
    ...
    stage("adapted-git-index", ("git", "-C", str(adapted), "add", "--all"), output, cwd=output)
    result = stage(
        "adapted-inventory",
        (str(command), "template", "list", "--root", str(adapted), "--format", "json"),
        output,
        cwd=output,
    ...
    result = stage(
        "adapted-surface",
        (str(command), "surface", "--root", str(adapted), "--format", "json"),
        output,
        cwd=output,
        environment=environment,
    )
...
def main() -> int:
    ...
    try:
        ...
        origin = stage(
            "development-install",
            (sys.executable, "-I", "-c", "import kicad_tooling; print(kicad_tooling.__file__)"),
            output,
            cwd=output,
        )
        ...
        if not args.skip_mdrepo:
            stage("mdrepo", (sys.executable, "-m", "mdrepo", "check", "."), output, cwd=ROOT)
        ...
        command = environment_dir / (
            "Scripts/kicad-team.exe" if os.name == "nt" else "bin/kicad-team"
        )
        ...
        origin = stage(
            "installed-origin",
            (str(python), "-I", "-c", "import kicad_tooling; print(kicad_tooling.__file__)"),
            output,
            cwd=output,
        ...
        if project is not None:
            inventory = stage(
                "external-inventory",
                (str(command), "template", "list", "--root", str(project), "--format", "json"),
                output,
                cwd=output,
                environment=external,
            )
            ...
            surface = stage(
                "external-surface",
                (str(command), "surface", "--root", str(project), "--format", "json"),
                output,
                cwd=output,
                environment=external,
            )
