...
def passing_doctor() -> TemplateDoctorReport:
    return TemplateDoctorReport(
        ...
        checks=(EnvironmentCheck(
            id="native-runner", required=True, status="PASS", expected="exact KiCad",
            observed="local", next_action="Run the selected board export.",
        ),),
        status="PASS",
    )


def evidence(args: tuple[str, ...], *, stdout: str = "", stderr: str = "",
             returncode: int = 0) -> CommandEvidence:
    return CommandEvidence(
        argv=("fake-kicad-cli", *args), started_utc="2026-09-24T00:00:00+00:00",
...
class VisualizeTests(unittest.TestCase):
    ...
    def test_check_models_is_read_only_and_retains_a_coachable_receipt(self) -> None:
        ...
        self.assertEqual(
            report.source_sha256[BOARD], hashlib.sha256(original).hexdigest(),
        )
    ...
    def test_broken_model_blocks_export_before_runner_selection(self) -> None:
        ...
        self.board.write_text(source.replace(
            marker, marker + '    (model "${KIPRJMOD}/models/missing.step")\n', 1,
        ), encoding="utf-8")
        with patch("kicad_tooling.hwrepo.three_d.doctor", side_effect=AssertionError("runner started")):
        ...
        self.assertIn("MODEL_PATH", {
            finding.code for finding in report.models.findings
        } if report.models else set())
        self.assertFalse(report.commands)
        self.assertFalse(report.artifacts_sha256)
        receipt = Path(report.run_directory)
        self.assertTrue((receipt / "models.json").is_file())
    ...
    def test_wrong_native_version_and_command_failure_keep_actionable_evidence(self) -> None:
        ...
        def wrong_version(_root: Path, _output: Path, _config: object, _selected: str,
        ...
        with patch("kicad_tooling.hwrepo.three_d.doctor", return_value=passing_doctor()), patch(
            "kicad_tooling.hwrepo.three_d._run_kicad", side_effect=wrong_version,
        ):
            version = generate(self.root, PROJECT, runner="local",
                               output=Path("build/wrong-version"))
        ...
        def failed_render(_root: Path, _output: Path, _config: object, _selected: str,
                          _cli: str, args: tuple[str, ...], timeout: int = 300) -> CommandEvidence:
            _ = timeout
            observed.append(args)
            if args == ("version",):
        ...
        observed.clear()
        with patch("kicad_tooling.hwrepo.three_d.doctor", return_value=passing_doctor()), patch(
            "kicad_tooling.hwrepo.three_d._run_kicad", side_effect=failed_render,
        ):
            failed = generate(self.root, PROJECT, runner="local",
                              output=Path("build/failed-render"))
    ...
    def test_success_exit_with_invalid_image_is_not_a_successful_export(self) -> None:
        def invalid_image(_root: Path, output: Path, _config: object, _selected: str,
                          _cli: str, args: tuple[str, ...], timeout: int = 300) -> CommandEvidence:
            _ = timeout
            if args == ("version",):
                return evidence(args, stdout="10.0.5\n")
        ...
        with patch("kicad_tooling.hwrepo.three_d.doctor", return_value=passing_doctor()), patch(
            "kicad_tooling.hwrepo.three_d._run_kicad", side_effect=invalid_image,
        ):
            report = generate(self.root, PROJECT, runner="local",
                              output=Path("build/invalid-image"))
        self.assertEqual(report.status, "FAIL")
    ...
    def test_successful_exports_are_hashed_and_source_rewrites_invalidate_them(self) -> None:
        ...
        def mutating_export(root: Path, output: Path, config: object, selected: str,
                            cli: str, args: tuple[str, ...], timeout: int = 300) -> CommandEvidence:
        ...
        with patch("kicad_tooling.hwrepo.three_d.doctor", return_value=passing_doctor()), patch(
            "kicad_tooling.hwrepo.three_d._run_kicad", side_effect=mutating_export,
        ):
            changed = generate(self.root, PROJECT, runner="local",
                               output=Path("build/source-changed"))
