...
class HostedCiTests(unittest.TestCase):
    ...
    def test_release_rehearsal_restores_examples_after_adoption_without_copying_live_data(self) -> None:
        ...
        private_paths = (
            "projects/private-board/design.txt", "products/private-product/design.txt",
            "libraries/private-library/design.txt", "hardware/private-board/design.txt",
        )
        for name in private_paths:
            path = self.root / name
            path.parent.mkdir(parents=True, exist_ok=True)
            path.write_text("Private live source must remain outside rehearsal.\n", encoding="utf-8")
        ...
        preserved = {name: (self.root / name).read_bytes() for name in (
            *private_paths, "kicad-tooling.toml", "template-adoption.json", "LICENSE",
            "catalog/projects.json", "examples/projects/arduino-uno-status-led/project.json",
        )}
        log = HostedLog(self.root, "release-fixture")
        run = log.run
        target = self.root / "build/rehearsal-source"

        def stop_before_native(stage: str, argv: tuple[str, ...], *, cwd: Path,
                               **kwargs: object) -> Path:
        ...
        with patch.object(log, "run", side_effect=stop_before_native), self.assertRaisesRegex(
        ...
        self.assertEqual((target / "catalog/projects.json").read_bytes(),
                         (self.root / "examples/catalog/projects.json").read_bytes())
        manifest = json.loads((target / "examples/projects/arduino-uno-status-led/project.json").read_text())
        self.assertEqual(manifest["release_exports"]["supplier_formats"], ["odb", "ipc2581", "ipcd356"])
