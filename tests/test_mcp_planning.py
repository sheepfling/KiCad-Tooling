...
class McpPlanningTests(unittest.IsolatedAsyncioTestCase):
    ...
    async def cli(self, module: str, *arguments: str) -> subprocess.CompletedProcess[str]:
        return await asyncio.to_thread(subprocess.run, (sys.executable, "-B", "-m", module, "--root", str(self.root),
                               *arguments), cwd=SOURCE_ROOT, capture_output=True, text=True,
                              check=False)
    ...
    def sourcing_snapshot(self) -> SourcingSnapshot:
        return SourcingSnapshot(
            snapshot_id="training-offer-observation", source_commit="a" * 40,
            observed_at=datetime(2026, 9, 7, tzinfo=UTC), offers=(SupplierOffer(
                id="offer-1", part_id="training-generic-led-red-5mm",
                supplier="Example supplier observation", supplier_sku="EXAMPLE-LED-5MM",
                source_url="https://example.invalid/offer", region="test-only", currency="USD",
                quantity_break=1, unit_price_minor=0, availability="not a purchasing instruction",
            ),),
        )
    ...
    async def test_impact_cli_and_mcp_match_paths_full_and_manual_modes(self) -> None:
        cases = (
            ...
            (("--path", "unowned.txt", "--path", "README.md"), {"paths": ["unowned.txt", "README.md"]}),
    ...
    async def test_impact_cli_and_mcp_match_git_refs_and_block_option_injection(self) -> None:
        ...
        await asyncio.to_thread(git, "-c", "user.name=Test fixture", "-c", "user.email=fixture@example.invalid",
            "commit", "-qm", "Initial synthetic source")
        ...
        await asyncio.to_thread(git, "-c", "user.name=Test fixture", "-c", "user.email=fixture@example.invalid",
            "commit", "-qam", "Changed selected synthetic board")
        async with Client(create_server(self.root), mode="legacy") as client:
            for reference in (base, "HEAD~1"):
                command = await self.cli("kicad_tooling.impact", "--base", reference, "--head", "HEAD")
                self.assertEqual(command.returncode, 0, command.stderr)
            ...
            result = await client.call_tool("plan_impact", {"base": base, "head": f"--output={destination}"})
            self.assertTrue(result.is_error)
    ...
    async def test_impact_invalid_modes_and_selectors_fail_in_both_interfaces(self) -> None:
        cases = (
            ((), {}), (("--full", "--select-project", "controller"), {"full": True, "select_project": "controller"}),
    ...
    async def test_model_map_cli_and_mcp_match_draft_content_and_authority(self) -> None:
        ...
        before = self.source_snapshot()
        command = await self.cli("kicad_tooling.visualize", "--project", project,
                           "--init-model-map", "build/cli-map/model-map.json",
                           "--output", "build/cli-map", "--format", "json")
        self.assertEqual(command.returncode, 0, command.stderr)
        expected = json.loads(command.stdout)
        async with Client(create_server(self.root, allow_exports=True), mode="legacy") as client:
            actual = self.structured(await client.call_tool("init_model_map", {
                "project_id": project, "view_id": "draft-one",
            }))
            excluded = {"draft_map", "run_directory", "next_commands"}
            self.assertEqual({key: value for key, value in actual.items() if key not in excluded},
            ...
            self.assertEqual(read_model(Path(actual["draft_map"]), ModelMap),
                             read_model(Path(expected["draft_map"]), ModelMap))
            draft = self.structured(await client.call_tool("read_artifact", {
                "path": "build/model-maps/draft-one/model-map.json",
            }))
            self.assertTrue(all(item["model"] == "" for item in json.loads(draft["text"])["assignments"]))
        self.assertEqual(self.source_snapshot(), before)
    ...
    async def test_model_map_failure_receipts_match_cli_and_gates_refuse_overwrite(self) -> None:
        ...
        command = await self.cli("kicad_tooling.visualize", "--project", project,
                           "--init-model-map", "build/cli-empty/model-map.json",
                           "--output", "build/cli-empty", "--format", "json")
        self.assertEqual(command.returncode, 1, command.stderr)
        expected = json.loads(command.stdout)
        async with Client(create_server(self.root), mode="legacy") as client:
            self.assertTrue((await client.call_tool("init_model_map", {
                "project_id": project, "view_id": "disabled",
            })).is_error)
        self.assertFalse((self.root / "build/model-maps/disabled").exists())
        async with Client(create_server(self.root, allow_exports=True), mode="legacy") as client:
            actual = self.structured(await client.call_tool("init_model_map", {
                "project_id": project, "view_id": "empty",
            }))
    ...
    async def test_sourcing_cli_and_mcp_match_artifact_and_semantic_failures(self) -> None:
        ...
        cases = (snapshot, snapshot.model_copy(update={"offers": snapshot.offers * 2}),
                 snapshot.model_copy(update={"offers": (snapshot.offers[0].model_copy(
                     update={"part_id": "unknown-part"}),)}))
        before = self.source_snapshot()
        async with Client(create_server(self.root), mode="legacy") as client:
            for current in cases:
                with self.subTest(offers=current.offers):
                    write_model(path, current)
                    command = await self.cli("kicad_tooling.sourcing", "--snapshot", "build/sourcing/snapshot.json")
                    expected = json.loads(command.stdout)
                    self.assertEqual(command.returncode, 0 if expected["status"] == "PASS" else 1)
                    actual = self.structured(await client.call_tool("inspect_sourcing_snapshot", {
    ...
    async def test_sourcing_load_failures_match_cli_and_paths_are_bounded(self) -> None:
        ...
        async with Client(create_server(self.root), mode="legacy") as client:
            for name in ("build/broken.json", "build/missing.json"):
                ...
                actual = self.structured(await client.call_tool("inspect_sourcing_snapshot", {"path": name}))
                self.assertEqual(actual, json.loads(command.stdout))
            ...
            for request in ({}, {"path": "../outside.json"}, {"path": str(path)},
                            {"path": "catalog/parts.json"}, {"path": "build/linked.json"},
                            {"snapshot": self.sourcing_snapshot().model_dump(mode="json")}):
                self.assertTrue((await client.call_tool("inspect_sourcing_snapshot", request)).is_error)
        self.assertEqual({entry.name for entry in (self.root / "build").iterdir()}, {"broken.json", "linked.json"})
    ...
    async def test_sourcing_special_file_is_rejected_before_reader_can_block(self) -> None:
        ...
        async with Client(create_server(self.root), mode="legacy") as client:
            with patch("kicad_tooling.hwrepo.mcp_planning.read_model") as reader:
                report = self.structured(await client.call_tool("inspect_sourcing_snapshot", {
