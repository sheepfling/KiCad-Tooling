...
class ReviewScopeParityTests(unittest.IsolatedAsyncioTestCase):
    ...
    def synthetic_native(
        self, _root, project, output, _cli, _dependencies, export_only=False, assembly_variant=None
    ):
        ...
        (output / filename).write_text(
            json.dumps({"synthetic_project": project.id}), encoding="utf-8"
        )
        if export_only:
    ...
    async def test_actual_cli_and_mcp_prepare_same_project_variant_scope(self) -> None:
        ...
        with (
            patch("kicad_tooling.hwrepo.releasing.run_native", side_effect=self.synthetic_native),
        ...
        self.assertEqual(
            (self.root / "build/releases/cli-scope" / product).read_bytes(),
                ...
                with patch("kicad_tooling.hwrepo.mcp_workflow.selected_cli") as runner:
                    with self.assertRaises(ValueError):
                        workflow.prepare_review_scope(
                            self.root, f"invalid-{index}", projects, variants
                        )
    ...
    def test_saved_portable_and_valid_native_evidence_support_same_package_lifecycle(self) -> None:
        ...
        with (
            ...
            result = workflow.prepare_review_scope(
                ...
                portable="build/releases/test/portable.json",
                runner="local",
            )
        ...
        self.assertEqual(
            workflow.package_release(self.root, manifest, "scope-package").status, "PASS"
        )
        self.assertEqual(
            workflow.verify_package(self.root, "build/packages/scope-package.zip").status, "PASS"
        )
        self.assertEqual(
            workflow.restore_package(
                self.root,
                "build/packages/scope-package.zip",
                "scope-restored",
            ).status,
            "PASS",
        )
    ...
    def test_container_scope_preserves_selection_and_captures_nonprotocol_stdout(self) -> None:
        ...
        expected = self.fixture.manifest.model_copy(
            update={
                "release_id": "container-scope",
                "projects": (project,),
                "variants": selection,
            }
        )
        ...
        receipt = read_model(
            self.root / "build/releases/container-scope.command.json", CommandEvidence
        )
