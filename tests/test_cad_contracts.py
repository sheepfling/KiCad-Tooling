...
def provider_document() -> dict:
    return {"success": True, "ignored_vendor_field": "allowed outside our projection", "result": {
        "lcsc": {"number": "C2040"},
        "dataStr": {"head": {"c_para": {"Manufacturer": "Raspberry Pi", "Manufacturer Part": "RP2040",
            "Supplier Part": "C2040", "name": "RP2040", "package": "QFN-56"}}},
        "packageDetail": {"dataStr": {"shape": ['PAD~example', 'SVGNODE~' + json.dumps({"attrs": {
            "uuid": "a" * 32, "title": "QFN-56"}})]}},
    }}
...
class CadContractsTests(unittest.TestCase):
    ...
    def test_missing_or_ambiguous_3d_pair_is_blocked(self) -> None:
        for shapes in ([], ["PAD~only"], "not a list", [False],
                       ["SVGNODE~{}", "SVGNODE~{}"], ["SVGNODE~{bad json}"],
                       ['SVGNODE~{"attrs":false}']):
    ...
    def test_identity_never_coerces_bad_provider_fields(self) -> None:
        for field, value in (("Manufacturer", ""), ("Manufacturer Part", 123),
                             ("Supplier Part", "../../C2040"), ("name", None), ("package", False)):
            raw = provider_document()
    ...
    def test_duplicate_keys_and_non_finite_json_are_rejected(self) -> None:
        ...
        for invalid in (document.replace('"success": true', '"success":true,"success":true'),
                        document.replace('"ignored_vendor_field": "allowed outside our projection"',
    ...
    def test_source_file_contract_rejects_unknown_fields_and_invalid_hashes(self) -> None:
        ...
        for invalid in (valid.replace('"path":', '"extra":1,"path":'),
                        valid.replace('0' * 64, 'changed'), valid.replace('"part.kicad_sym"', 'false')):
            with self.subTest(document=invalid), self.assertRaises(ValueError):
