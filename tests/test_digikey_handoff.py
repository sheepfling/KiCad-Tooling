...
def line(*, part_id: str = "RES-1", order_number: str = "311-1.00KHRCT-ND",
         manufacturer: str = "Yageo", mpn: str = "RC0603FR-071KL",
         kind: str = "DigiKey") -> PurchasingLine:
    return PurchasingLine(
        part_id=part_id, revision="A", manufacturer=manufacturer, mpn=mpn,
        footprint="Resistor_SMD:R_0603_1608Metric", references=("R1", "R2"),
        per_board=2, required=6, spares=2, quantity=8, order_number=order_number,
        order_number_kind=kind, search_url="https://www.digikey.com/en/products",
    )


def ready(*lines: PurchasingLine) -> PurchasingPlan:
    return PurchasingPlan(
        status="READY_FOR_ORDER_REVIEW", preferences=PurchasingPreferences(boards=3),
        components=(), lines=lines or (line(),), findings=(), excluded_references=("R9",),
    )
...
class DigiKeyHandoffTests(unittest.TestCase):
    def test_payload_preserves_exact_order_number_quantities_and_identity(self) -> None:
        ...
        self.assertEqual(result.model_dump(mode="json", by_alias=True), [{
            "requestedPartNumber": "311-1.00KHRCT-ND", "quantities": [{"quantity": 8}],
    ...
    def test_provider_contract_round_trip_and_strict_rejection(self) -> None:
        ...
        for document in (
            ...
            serialized.replace('"requestedPartNumber":"311-1.00KHRCT-ND"',
                               '"requestedPartNumber":12'),
        ):
    ...
    def test_incomplete_or_empty_plan_cannot_send(self) -> None:
        finding = PurchasingFinding(code="missing", message="Missing identity", action="Choose part")
        for plan in (ready().model_copy(update={"status": "NEEDS_PARTS"}),
    ...
    def test_ambiguous_identical_mpn_needs_exact_supplier_numbers(self) -> None:
        ...
        second = line(part_id="RES-2", order_number="123", mpn="123",
                      manufacturer="Other", kind="MPN")
        with self.assertRaisesRegex(DigiKeyHandoffError, "different manufacturers"):
            build_payload(ready(first, second))

    def connection(self, body: bytes = b'"https://www.digikey.com/short/abc1234"',
    ...
    def test_single_https_post_uses_documented_noauth_schema(self) -> None:
        ...
        with patch("kicad_tooling.hwrepo.digikey_handoff.http.client.HTTPSConnection",
        ...
        self.assertEqual(parse_qs(url.query), {
            "listName": ["Board & prototype / 2"], "tags": ["KiCad-Parts-Assistant"],
        })
        submitted = DigiKeyHandoffPayload.model_validate_json(
            connection.request.call_args.kwargs["body"], strict=True,
        )
    ...
    def test_timeout_does_not_retry_an_ambiguous_post(self) -> None:
        ...
        with patch("kicad_tooling.hwrepo.digikey_handoff.http.client.HTTPSConnection",
                   return_value=connection), self.assertRaisesRegex(
                       DigiKeyHandoffError, "No automatic retry"):
    ...
    def test_response_trickle_cannot_reset_total_deadline(self) -> None:
        ...
        with patch("kicad_tooling.hwrepo.digikey_handoff.http.client.HTTPSConnection",
                   return_value=connection), patch(
                       "kicad_tooling.hwrepo.digikey_handoff.time.monotonic", side_effect=ticks,
    ...
    def test_untrusted_or_unsupported_response_never_becomes_a_link(self) -> None:
        for body in (
            b'<html>Just a moment</html>', b'null', b'{}',
            ...
            b'"https://www.digikey.com/short/abc1234/extra"', b'\xff',
        ):
