        ...
        def normalize(value, field=None):
            ...
            if isinstance(value, str):
                return (
                    ...
                    else value.replace(str(output), "<output>")
                )
            return value
...
"""
    ...
    async def test_release_readiness_and_archive_lifecycle_parity(self) -> None:
        ...
        async with Client(create_server(self.root, allow_exports=True), mode="legacy") as client:
            mcp_ready = await self.call(
                client, "check_release", ReleaseReadinessReport, {"manifest": fixture.manifest_name}
            )
            ...
            mcp_package = await self.call(
                client,
                "package_release",
                ReleasePackageReport,
                {
                    "manifest": fixture.manifest_name,
                    "package_id": "mcp-review",
                },
            )
            ...
            self.assertEqual(
                cli_package.model_copy(
                    update={"package": "<archive>", "package_sha256": "<zip-container>"}
                ),
                mcp_package.model_copy(
                    update={"package": "<archive>", "package_sha256": "<zip-container>"}
                ),
            )
            for archive in (cli_archive, mcp_archive):
                with self.subTest(archive=archive.name):
                    cli_verified = await self.cli(
                        "kicad_tooling.release",
                        ReleasePackageReport,
                        "verify",
                        "--archive",
            ...
            mcp_restored = await self.call(
                client,
                "restore_package",
                ReleasePackageReport,
                {
                    "archive": mcp_archive.relative_to(self.root).as_posix(),
                    "restore_id": "mcp-review",
                },
            )
            ...
            mcp_denied = await client.call_tool(
                "package_release",
                {
                    "manifest": fixture.manifest_name,
                    "package_id": "mcp-stale",
                },
            )
        ...
        with patch.dict(
            ...
            async with Client(
                ...
                for invalid in (False, True):
                    with (
                        self.subTest(invalid_image=invalid),
                        patch.dict(
                            os.environ,
                            {"PARITY_INVALID_IMAGE": "1" if invalid else "0"},
                        ),
                    ):
                        cli_directory = self.root / (
                            "build/3d/cli-invalid" if invalid else "build/3d/cli-good"
                        )
                        cli = await self.cli(
                            "kicad_tooling.visualize",
                            ThreeDReport,
                            "--project",
                            test_visualize.PROJECT,
                            "--runner",
                            "local",
                            "--output",
                            str(cli_directory),
                            expected_exit=int(invalid),
                        )
                        mcp = await self.call(
                            client,
                            "export_3d",
                        ...
                        self.assertEqual(
                            self.semantic(cli, cli_directory), self.semantic(mcp, mcp_directory)
                        )
    ...
    def configure_variants(self) -> None:
        ...
        self.fixture.git(
            ...
            "commit",
            "-qam",
            "Synthetic variant configuration",
        )
            ...
            async with Client(
                ...
                for suffix, variant in (("default", None), ("explicit", "Pilot B")):
                    with self.subTest(variant=variant):
                        ...
                        self.assertEqual(
                            self.export_semantic(cli, cli_directory),
                ...
                mcp_3d = await self.call(
                    ...
                    ThreeDReport,
                    {
                        ...
                        "view_id": "mcp-variant",
                        "runner": "local",
                        "assembly_variant": "Pilot B",
                    },
                )
                self.assertEqual(
                    self.semantic(cli_3d, cli_directory),
        ...
        async with Client(
            ...
            for index, variant in enumerate(("Missing population", " ")):
                with (
                    ...
                    patch("kicad_tooling.hwrepo.mcp_workflow.selected_cli") as select,
                ):
                    result = await client.call_tool(
                        "export_project",
                        {
                            "project_id": test_visualize.PROJECT,
                            "export_id": f"invalid-{index}",
                            "runner": "local",
                            "assembly_variant": variant,
                        },
                    )
                    ...
                    command = await self.cli_process(
                        "kicad_tooling.release",
                        "export",
                        "--project",
                        test_visualize.PROJECT,
            ...
            self.assertEqual(
                self.semantic(cli_3d, Path(cli_3d.run_directory)),
                self.semantic(mcp_3d, Path(mcp_3d.run_directory)),
            )
    ...
    async def test_native_fabrication_export_parity(self) -> None:
        ...
        with patch.dict(
            ...
            async with Client(
                ...
                for suffix, failing, mutating in (
                    ("good", False, False),
                    ("failed", True, False),
                    ("mutated", False, True),
                ):
                    with (
                        self.subTest(scenario=suffix),
                        patch.dict(
                            os.environ,
                            {
                                "PARITY_FAILED_EXPORT": "1" if failing else "0",
                        ...
                        cli = await self.cli(
                            ...
                            "--output",
                            str(cli_directory),
                            expected_exit=int(failing or mutating),
                        )
                        if mutating:
                        ...
                        for name in (
                            "fabrication/board.drl",
                            "assembly/positions.csv",
                            "assembly/bom.csv",
                            "assembly/purchasing-bom.csv",
                        ):
                            self.assertEqual(
                                (cli_directory / name).read_bytes(),
                                (mcp_directory / name).read_bytes(),
                        ...
                        if failing:
                            ...
                            self.assertIn(
                                "Synthetic Gerber command failure", mcp.commands["gerbers"].stderr
                            )
                            self.assertFalse((mcp_directory / "fabrication/board.gbr").exists())
                        else:
                            self.assertEqual(
                                (cli_directory / "fabrication/board.gbr").read_bytes(),
                                (mcp_directory / "fabrication/board.gbr").read_bytes(),
                            )
                        if mutating:
                            self.assertNotEqual(board.read_bytes(), original_board)
                            self.assertTrue(
                                all(command.returncode == 0 for command in mcp.commands.values())
                            )
                            board.write_bytes(original_board)
                        self.assertEqual(source_state(self.root), before)
