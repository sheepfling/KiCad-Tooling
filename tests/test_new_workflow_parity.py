...
class NewWorkflowParityTests(unittest.IsolatedAsyncioTestCase):
    def cad_fixture(self) -> tuple[cad_fixture.CadLibraryTests, CadSourceReport]:
        ...
        source = CadSourceReport(
            status="READY", supplier_id="C1234", bundle_directory=str(cache),
            bundle=fixture.bundle, receipt_directory=str(fixture.root / "build/cad-source/saved"),
        )
    ...
    def cli_parts(root: Path, *args: str) -> tuple[int, dict[str, object]]:
        output = StringIO()
        with (patch.object(sys, "argv", ["kicad_tooling.parts", "--root", str(root),
                                            "--project", "controller", *args,
    ...
    def project_bytes(root: Path) -> dict[str, bytes]:
        ...
        return {path.relative_to(base).as_posix(): path.read_bytes()
                for path in base.rglob("*") if path.is_file() and "build" not in path.parts}
    ...
    async def test_saved_electrical_charts_and_suite_match_cli(self) -> None:
        ...
        process = await asyncio.to_thread(subprocess.run, (sys.executable, "-I", "-B", "-m", "kicad_tooling.electrical_charts",
                                 ...
                                 timeout=90, env={**os.environ, "MPLCONFIGDIR": str(root / "build/matplotlib")})
        ...
        async with Client(create_server(root, allow_exports=True), mode="legacy") as client:
            response = await client.call_tool("export_electrical_charts", {
                "receipt": source, "view_id": "mcp"})
            self.assertFalse(response.is_error, response.content)
            mcp = ElectricalChartsReport.model_validate_json(json.dumps(response.structured_content))
            self.assertEqual([(item.id, item.status, item.samples, item.waveform_sha256,
                               item.csv, item.png, item.svg) for item in cli.cases],
                             [(item.id, item.status, item.samples, item.waveform_sha256,
                               item.csv, item.png, item.svg) for item in mcp.cases])
            ...
            for case in cli.cases:
                ...
                self.assertEqual((Path(cli.run_directory) / case.csv).read_bytes(),
            ...
            response = await client.call_tool("export_electrical_chart_suite", {
                "suite": "build/electrical-suite.json", "view_id": "mcp-suite"})
            self.assertFalse(response.is_error, response.content)
            mcp_suite = ElectricalChartsSuiteReport.model_validate_json(json.dumps(response.structured_content))
            self.assertEqual(cli_suite.status, mcp_suite.status)
            self.assertEqual([(item.project_id, item.status) for item in cli_suite.reports],
                             [(item.project_id, item.status) for item in mcp_suite.reports])
            outside = await client.call_tool("export_electrical_charts", {
    ...
    async def test_exact_cad_lookup_preview_and_apply_match_cli(self) -> None:
        ...
        async with Client(create_server(mcp_fixture.root, allow_exports=True, allow_edits=True),
            ...
            self.assertEqual((cli_plan.status, cli_plan.symbol_id, cli_plan.footprint_id,
                              cli_plan.files, cli_plan.check, cli_plan.diff),
                             (mcp_plan.status, mcp_plan.symbol_id, mcp_plan.footprint_id,
                              mcp_plan.files, mcp_plan.check, mcp_plan.diff))
            self.assertEqual(cli_plan.status, "PLAN")
            assert cli_plan.plan_path is not None and mcp_plan.plan_path is not None
            cli_status, cli_value = self.cli_parts(cli_fixture.root, "--import-cad",
                                                    cli_plan.plan_path, "--apply")
            ...
            result = await client.call_tool("apply_cad_import", {
                "project_id": "controller", "view_id": "apply", "plan": mcp_plan_relative,
                "expected_sha256": expected})
            self.assertFalse(result.is_error, result.content)
            mcp_applied = CadImportReport.model_validate_json(json.dumps(result.structured_content))
            self.assertEqual((cli_applied.status, cli_applied.files, cli_applied.symbol_id,
                              cli_applied.footprint_id),
                             (mcp_applied.status, mcp_applied.files, mcp_applied.symbol_id,
                              mcp_applied.footprint_id))
            self.assertEqual(cli_applied.status, "APPLIED")
            self.assertEqual(self.project_bytes(cli_fixture.root),
                             self.project_bytes(mcp_fixture.root))
            replay = await client.call_tool("apply_cad_import", {
                "project_id": "controller", "view_id": "replay", "plan": mcp_plan_relative,
                "expected_sha256": expected})
            self.assertFalse(replay.is_error, replay.content)
            self.assertEqual(CadImportReport.model_validate_json(json.dumps(replay.structured_content)).status,
                             "BLOCKED")

    async def test_network_permission_and_step_review_match_cli(self) -> None:
        fixture, source = self.cad_fixture()
        root = fixture.root
        async with Client(create_server(root, allow_exports=True, allow_checks=True),
                          mode="legacy") as client:
            ...
            self.assertFalse(result.is_error, result.content)
            self.assertEqual(CadSourcingReview.model_validate_json(json.dumps(result.structured_content)).source.status,
                             "BLOCKED")
            self.assertFalse((root / "build/cad-source-cache/easyeda-1.0.1/C9999/current.txt").exists())
            review = CadStepReport(status="REVIEW", project_id="controller",
                                   supplier_id="C1234", receipt_directory="synthetic",
                                   issues=("Inspect paired views",))
            with (patch("kicad_tooling.hwrepo.cad_source.fetch", return_value=source),
                  patch("kicad_tooling.hwrepo.cad_step.review", return_value=review)):
                cli_status, cli_value = self.cli_parts(root, "--check-step", "C1234")
                response = await client.call_tool("check_step_alignment", {
                    "project_id": "controller", "view_id": "step", "supplier_id": "C1234"})
            self.assertEqual(cli_status, 0)
            self.assertFalse(response.is_error, response.content)
            self.assertEqual(CadStepReport.model_validate_json(json.dumps(cli_value)),
