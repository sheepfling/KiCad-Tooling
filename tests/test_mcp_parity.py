...
class McpParityTests(unittest.IsolatedAsyncioTestCase):
    ...
    def semantic(report: BaseModel, receipt: str | None = None):
        def normalize(value, field=None):
            ...
            if isinstance(value, str):
                ...
                if field == "stderr":
                    value = re.sub(r"(?m)^(Ran \d+ tests? in )\d+(?:\.\d+)?s$",
                                   r"\1<elapsed>s", value)
            return value
    ...
    def import_args(self):
        return {"source": self.incoming.relative_to(self.root).as_posix(),
                "project_id": "incoming", "toolchain_id": "kicad-10.0.5"}

    def damage_import(self) -> None:
        self.incoming.with_suffix(".kicad_sch").write_text(
            '(kicad_sch (sheet (property "Sheetfile" "missing.kicad_sch")))', encoding="utf-8",
        )
    ...
    def native_evidence(self) -> Path:
        ...
        write_model(self.root / "catalog/parts.json", PartsCatalog(
            schema_version="0.1", parts=(PartRecord(
                id="resistor-1k", revision="A", description="Synthetic parity-test identity",
        ...
        write_model(directory / "summary.json", ValidationSummary(
        ...
        async with Client(create_server(self.root), mode="legacy") as client:
            for broken in (False, True):
                with self.subTest(malformed_peer=broken):
                    ...
                    cli = await self.cli("kicad_tooling.template", TemplateInventoryReport, "list",
    ...
    async def test_doctor_failure_parity(self) -> None:
        ...
        with patch.dict(os.environ, {"PATH": ""}):
            cli = await self.cli("kicad_tooling.template", TemplateDoctorReport, "doctor", "--native",
                                 "--project-id", "controller", "--runner", "local", expected_exit=1)
            async with Client(create_server(self.root), mode="legacy") as client:
                mcp = await self.call(client, "doctor", TemplateDoctorReport, {
                    "project_id": "controller", "native": True, "runner": "local",
                })
        self.assertEqual(cli, mcp)
        ...
        async with Client(create_server(self.root), mode="legacy") as client:
            for broken in (False, True):
                with self.subTest(missing_sheet=broken):
                    if broken:
                        self.damage_import()
                    cli = await self.cli("kicad_tooling.template", ProjectImportReport, "import-project",
                                         "--source", str(self.incoming), "--project-id", "incoming",
                                         "--toolchain", "kicad-10.0.5", "--dry-run",
                                         expected_exit=int(broken))
                    mcp = await self.call(client, "preview_import", ProjectImportReport,
                                          self.import_args())
                    self.assertEqual(cli, mcp)
                    self.assertEqual(mcp.status, "FAIL" if broken else "PASS")
    ...
    async def test_import_scan_parity(self) -> None:
        ...
        cli = await self.cli("kicad_tooling.template", ImportInventoryReport, "scan-imports",
                             "--source-dir", str(self.incoming.parent), "--toolchain",
                             "kicad-10.0.5", expected_exit=1)
        async with Client(create_server(self.root), mode="legacy") as client:
            mcp = await self.call(client, "scan_imports", ImportInventoryReport, {
    ...
    async def test_import_diagnosis_parity(self) -> None:
        ...
        cli = await self.cli("kicad_tooling.template", DiagnosticReport, "diagnose", "--source",
                             str(self.incoming), "--project-id", "incoming", "--toolchain",
                             "kicad-10.0.5", expected_exit=1)
        async with Client(create_server(self.root), mode="legacy") as client:
    ...
    async def test_rescue_parity(self) -> None:
        (self.root / "examples/projects/passive-signal-reference/project.json").write_text("{bad")
        cli = await self.cli("kicad_tooling.template", LocalRescueReport, "rescue", "--project-id",
                             "controller", expected_exit=1)
        async with Client(create_server(self.root), mode="legacy") as client:
            mcp = await self.call(client, "rescue_project", LocalRescueReport,
                                  {"project_id": "controller"})
        self.receipt_equal(cli, mcp)
        ...
        async with Client(create_server(self.root, allow_checks=True), mode="legacy") as client:
            for broken in (False, True):
                with self.subTest(missing_source=broken):
                    if broken:
                        (self.island / "kicad/controller.kicad_sch").unlink()
                    cli = await self.cli("kicad_tooling.verify", ProjectVerificationReport,
                                         "--project", "controller", expected_exit=int(broken))
                    mcp = await self.call(client, "check_project", ProjectVerificationReport,
                                          {"project_id": "controller"})
                    self.receipt_equal(cli, mcp)
                    self.assertEqual(mcp.status, "FAIL" if broken else "PASS")
                    self.assertFalse(mcp.build_authorized)
    ...
    async def test_tag_shard_scope_check_parity(self) -> None:
        cli = await self.cli("kicad_tooling.ci", ProjectStaticPipelineReport,
    ...
    async def test_native_scope_failure_parity(self) -> None:
        ...
        with patch.dict(os.environ, {"PATH": str(binary) + os.pathsep + os.environ.get("PATH", "")}):
            cli = await self.cli("kicad_tooling.ci", CheckAllSummary, "--kicad", "--project", "controller",
                                 "--output", str(self.root / "build/native-cli"), expected_exit=1)
            async with Client(create_server(self.root, allow_checks=True), mode="legacy") as client:
                result = await client.call_tool("check_native_scope", {
                    "view_id": "native-mcp", "project_ids": ["controller"],
                })
        self.assertFalse(result.is_error, result.content)
        self.assertIsNotNone(result.structured_content)
        ...
        self.assertEqual(result.structured_content["status"], cli.status)
        self.assertFalse(result.structured_content["build_authorized"])
        cli_native = read_model(self.root / "build/native-cli/controller/summary.json",
                                ValidationSummary)
        mcp_native = read_model(Path(result.structured_content["run_directory"])
                                / "controller/summary.json", ValidationSummary)
    ...
    async def test_new_project_parity(self) -> None:
        cli = await self.cli("kicad_tooling.template", ProjectScaffoldReport, "new-project", "--project-id",
                             "fresh-board", "--kind", "pcb_only", "--toolchain", "kicad-10.0.5")
        directory = self.root / "projects/fresh-board"
        expected_files = {path.relative_to(directory).as_posix(): path.read_bytes()
                          for path in directory.rglob("*") if path.is_file()}
        shutil.rmtree(directory)
        async with Client(create_server(self.root, allow_writes=True), mode="legacy") as client:
            mcp = await self.call(client, "new_project", ProjectScaffoldReport, {
                "project_id": "fresh-board", "kind": "pcb_only", "toolchain_id": "kicad-10.0.5",
            })
        self.assertEqual(cli, mcp)
        self.assertEqual(expected_files, {path.relative_to(directory).as_posix(): path.read_bytes()
                                         for path in directory.rglob("*") if path.is_file()})

    async def test_import_write_parity(self) -> None:
        cli = await self.cli("kicad_tooling.template", ProjectImportReport, "import-project", "--source",
                             str(self.incoming), "--project-id", "incoming", "--toolchain",
                             "kicad-10.0.5")
        ...
        async with Client(create_server(self.root), mode="legacy") as client:
            for tampered in (False, True):
                with self.subTest(tampered=tampered):
                    ...
                    cli = await self.cli("kicad_tooling.contract_coach", ContractCoachReport, "--project-id",
                                         "controller", "--native-summary", str(native),
                                         expected_exit=int(tampered))
                    mcp = await self.call(client, "inspect_contract", ContractCoachReport, {
                        "project_id": "controller", "native_summary": native.relative_to(self.root).as_posix(),
                    })
                    self.assertEqual(cli, mcp)
        ...
        async with Client(create_server(self.root), mode="legacy") as client:
            for broken in (False, True):
                with self.subTest(missing_assigned_model=broken):
                    ...
                    board.write_text('(kicad_pcb (footprint "Test:Part" '
                                     f'(property "Reference" "U1") {assignment}))', encoding="utf-8")
                    cli = await self.cli("kicad_tooling.visualize", ThreeDReport, "--project", "controller",
                                         "--check-models", expected_exit=int(broken))
                    mcp = await self.call(client, "inspect_3d_models", ModelInventoryReport,
                                          {"project_id": "controller"})
    ...
    async def test_parts_preferences_parity(self) -> None:
        ...
        cli = await self.cli("kicad_tooling.parts", PurchasingPreferences, "--project", "controller",
                             "--init-preferences", str(path), "--boards", "10", "--spare-percent",
                             "10", "--spare-minimum", "3")
        expected_bytes = path.read_bytes()
        path.unlink()
        async with Client(create_server(self.root, allow_edits=True), mode="legacy") as client:
            mcp = await self.call(client, "save_parts_preferences", McpPurchasingPreferencesResult, {
                "project_id": "controller", "preferences": cli.model_dump(mode="json"),
            })
        ...
        path.unlink()
        cli_created = await self.cli("kicad_tooling.parts", McpPurchasingPreferencesResult,
                                     "--project", "controller", "--save-preferences", "--boards", "10",
                                     "--spare-percent", "10", "--spare-minimum", "3")
        original = path.read_bytes()
        path.unlink()
        async with Client(create_server(self.root, allow_edits=True), mode="legacy") as client:
            created = await self.call(client, "save_parts_preferences", McpPurchasingPreferencesResult, {
                "project_id": "controller", "preferences": cli_created.preferences.model_dump(mode="json"),
            })
            self.assertEqual(cli_created, created)
            self.assertEqual(path.read_bytes(), original)
            cli_updated = await self.cli("kicad_tooling.parts", McpPurchasingPreferencesResult,
                                         "--project", "controller", "--save-preferences", "--boards", "20",
                                         "--expected-sha256", created.after_sha256)
            updated_bytes = path.read_bytes()
            path.write_bytes(original)
            updated = await self.call(client, "save_parts_preferences", McpPurchasingPreferencesResult, {
                "project_id": "controller", "preferences": cli_updated.preferences.model_dump(mode="json"),
            ...
            rejected_cli = await self.cli_process(
                "kicad_tooling.parts", "--project", "controller", "--save-preferences", "--boards", "30",
                "--expected-sha256", created.after_sha256,
            )
            self.assertEqual(rejected_cli.returncode, 2)
            rejected_mcp = await client.call_tool("save_parts_preferences", {
                "project_id": "controller", "preferences": {"boards": 30},
                "expected_sha256": created.after_sha256,
            })
    ...
    async def test_parts_quantities_and_native_failure_parity(self) -> None:
        native = self.native_evidence()
        write_model(self.island / "docs/purchasing.json", PurchasingPreferences(
            boards=4, spare_percent=10, spare_minimum=3,
        ))
        cli = await self.cli("kicad_tooling.parts", PurchasingReport, "--project", "controller",
                             "--native-summary", str(native), "--boards", "10", "--output",
        ...
        async with Client(create_server(self.root, allow_exports=True), mode="legacy") as client:
            mcp = await self.call(client, "prepare_parts", PurchasingReport, {
                "project_id": "controller", "view_id": "mcp-quantities", "boards": 10,
        ...
        for name in ("bom.csv", "digikey.csv", "netlist.xml"):
            self.assertEqual((Path(cli.receipt_dir) / name).read_bytes(),
    ...
    async def test_parts_stale_evidence_parity(self) -> None:
        ...
        cli = await self.cli("kicad_tooling.parts", PurchasingReport, "--project", "controller",
                             "--native-summary", str(native), "--output",
                             str(self.root / "build/parts/cli-stale"), expected_exit=1)
        async with Client(create_server(self.root, allow_exports=True), mode="legacy") as client:
            mcp = await self.call(client, "prepare_parts", PurchasingReport, {
                "project_id": "controller", "view_id": "mcp-stale",
                "native_summary": native.relative_to(self.root).as_posix(),
            })
        self.receipt_equal(cli, mcp, "receipt_dir")
