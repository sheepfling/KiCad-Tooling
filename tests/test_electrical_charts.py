...
class ElectricalChartsTests(unittest.TestCase):
    ...
    def raw(self, name: str, names: tuple[tuple[str, str], ...],
            points: tuple[tuple[complex, ...], ...], *, complex_mode: bool = False) -> None:
        directory = self.source / name
        directory.mkdir()
        lines = ["Title: saved test waveform", f"Flags: {'complex' if complex_mode else 'real'}",
                 f"No. Variables: {len(names)}", f"No. Points: {len(points)}", "Variables:"]
        lines.extend(f"{index}\t{label}\t{unit}" for index, (label, unit) in enumerate(names))
        lines.append("Values:")
        for index, values in enumerate(points):
            lines.append(str(index))
            lines.extend(f"{value.real:.17g},{value.imag:.17g}" if complex_mode else
                         f"{value.real:.17g}" for value in values)
        (directory / "waveforms.raw").write_text("\n".join(lines) + "\n")

    def complete(self) -> ElectricalAnalysisReport:
        self.raw("startup", (("time", "time"), ("i(value0)", "current")),
                 ((0j, 0j), (0.001+0j, 4.9+0j), (0.005+0j, .05+0j)))
        self.raw("steady-state", (("time", "time"), ("i(value0)", "current"),
                                  ("value1", "power"), ("v(value2)", "voltage")),
                 ((0j, 0j, 0j, 0j), (.004+0j, .05+0j, .25+0j, 4.95+0j),
                  (.005+0j, .05+0j, .25+0j, 4.95+0j)))
        self.raw("passband", (("frequency", "frequency"), ("value0", "decibel"),
                              ("v(in)", "voltage"), ("v(out)", "voltage")),
                 ((1000+0j, -.001+0j, 1+0j, 1-.001j),
                  (1000000+0j, -.002+0j, 1+0j, 1-.002j),
                  (1000000000+0j, -3+0j, 1+0j, .7-.7j)), complex_mode=True)
        self.raw("edges", (("time", "time"), ("v(value0)", "voltage")),
                 ((0j, 0j), (.000001+0j, 1+0j), (.000003+0j, 1+0j)))
        artifacts = {path.relative_to(self.source).as_posix(): digest(path)
                     for path in self.source.rglob("*") if path.is_file()}
        report = ElectricalAnalysisReport(project_id=PROJECT, status="PASS",
                                          run_directory=str(self.source),
                                          artifacts_sha256=artifacts, checks=())
    ...
    def test_exports_all_cases_and_full_precision_complex_csv(self) -> None:
        ...
        self.assertEqual({item.id for item in result.cases}, {"startup", "steady-state", "passband", "edges"})
        ...
        self.assertEqual(float(rows[1][-1]), -.001)
    ...
    def test_missing_waveform_is_partial_and_suites_are_accepted(self) -> None:
        ...
        write_model(self.source / "electrical.json", report.model_copy(update={"artifacts_sha256": artifacts,
                                                                          "status": "FAIL"}))
        (self.source / "edges/waveforms.raw").unlink()
        partial = build_charts(self.root, self.source)
        self.assertEqual(partial.status, "PARTIAL")
        suite = ElectricalSuiteReport(status="FAIL", projects=(read_model(
            self.source / "electrical.json", ElectricalAnalysisReport),))
        ...
        self.assertEqual(read_model(Path(batch.run_directory) / "suite.json",
                                    ElectricalChartsSuiteReport), batch)
    ...
    def test_source_mutation_during_render_rejects_output_report(self) -> None:
        ...
        with (patch("kicad_tooling.hwrepo.electrical_charts.render_case", side_effect=change_after_render),
