...
class PickerRunner:
    ...
    def version(self, root: Path, config: ProjectConfig) -> CommandEvidence:
        return CommandEvidence(argv=("test-kicad-cli",), started_utc="2026-09-24T00:00:00+00:00",
                               returncode=0, stdout=config.kicad_version + "\n")

    def export(self, root: Path, config: ProjectConfig, output: Path) -> CommandEvidence:
        output.write_text(NETLIST, encoding="utf-8")
        return CommandEvidence(argv=("test-kicad-cli",), started_utc="2026-09-24T00:00:00+00:00", returncode=0)
...
class PartPickerTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.part = PartRecord(id="test-r1", revision="A", description="TEST-ONLY purchasing fixture",
            part_class="resistor", unit="each", manufacturer="Vishay", mpn="MRS25000C1001FCT00",
            datasheet_url="https://example.invalid/test-only", lifecycle="active", status=PartStatus.APPROVED,
            cad=PartCadBinding(symbol_id="Pilot:R", value="1k", footprint="Pilot:R_Test",
                model=self.model.relative_to(self.root).as_posix(), digikey_sku="541-TEST-ND"))
    ...
    def draft(self, report: PartPickerReport | None = None) -> Path:
        ...
        spec = report.selection_template.model_copy(update={
    ...
    def test_empty_training_missing_and_wrong_cad_catalogs_are_actionable(self) -> None:
        ...
        for record in (
            None, self.part.model_copy(update={"status": PartStatus.TRAINING}),
            self.part.model_copy(update={"cad": None}),
            self.part.model_copy(update={"cad": self.part.cad.model_copy(update={"symbol_id": "Device:R"})}),
            self.part.model_copy(update={"cad": self.part.cad.model_copy(update={"value": "10k"})}),
            self.part.model_copy(update={"cad": self.part.cad.model_copy(update={"model": "outside/missing.step"})}),
    ...
    def test_preview_has_all_diffs_and_does_not_write_authoritative_source(self) -> None:
        selected = self.draft()
        originals = {path: path.read_bytes() for path in (self.schematic, self.board, self.manifest_path)}
    ...
    def test_apply_updates_only_selected_sources_manifest_and_saved_supplier(self) -> None:
        write_model(self.preferences, PurchasingPreferences(boards=7, spare_percent=10, spare_minimum=3))
        report = selection(self.root, self.project_id, self.draft(), self.receipt())
        assert report.locked_map is not None
        applied = selection(self.root, self.project_id, Path(report.locked_map), self.receipt(), apply=True)
    ...
    def test_unlocked_wrong_project_removed_binding_and_duplicate_selection_block(self) -> None:
        ...
        for bad in (
            ...
            spec.model_copy(update={"assignments": (PartSelectionAssignment(reference="R2", part_id="test-r1"),)}),
            spec.model_copy(update={"assignments": (PartSelectionAssignment(reference="R1", part_id="missing"),)}),
        ...
        self.assertEqual(selection(self.root, self.project_id, selected, self.receipt(), apply=True).status, "BLOCKED")

    def test_all_source_policy_model_and_absent_preferences_drift_block(self) -> None:
        selected = self.draft()
        for path in (self.schematic, self.board, self.model, self.catalog_path,
                     self.root / "catalog/libraries.json", self.manifest_path,
    ...
    def test_locked_after_hashes_and_assignments_cannot_be_changed(self) -> None:
        ...
        write_model(path, locked.model_copy(update={"after_hashes": {name: "0" * 64 for name in locked.after_hashes}}))
    ...
    def test_transaction_rolls_back_every_applied_file_when_replace_fails(self) -> None:
        ...
        with patch("kicad_tooling.hwrepo.part_picker.os.replace", side_effect=fail_second):
            result = selection(self.root, self.project_id, Path(report.locked_map), self.receipt(), apply=True)
        self.assertEqual(result.status, "BLOCKED")
        self.assertFalse(self.preferences.exists())
        self.assertEqual({path: path.read_bytes() for path in originals}, originals)
    ...
    def test_pending_footprint_can_resume_after_f8_without_guessing_a_model(self) -> None:
        ...
        applied = selection(self.root, self.project_id, Path(preview.locked_map), self.receipt(), apply=True)
        ...
        self.board.write_text(self.board.read_text().replace('(footprint "Pilot:R_Test"', '(footprint "Pilot:R_New"', 1), encoding="utf-8")
        ...
        finished = selection(self.root, self.project_id, Path(resumed.locked_map), self.receipt(), apply=True)
    ...
    def test_undeclared_models_and_unsafe_output_do_not_bypass_capture(self) -> None:
        manifest = read_model(self.manifest_path, ProjectManifest)
        write_model(self.manifest_path, manifest.model_copy(update={
            "required_inputs": tuple(name for name in manifest.required_inputs if name != "kicad/test-only.step"),
        }))
        report = self.picker()
        self.assertEqual(report.status, "BLOCKED")
    ...
    def test_saved_supplier_conflicts_require_review_and_resume_needs_selection(self) -> None:
        write_model(self.preferences, PurchasingPreferences(digikey_skus={"test-r1": "DIFFERENT-ND"}))
        report = selection(self.root, self.project_id, self.draft(), self.receipt())
        self.assertEqual(report.status, "BLOCKED")
    ...
    def test_manifest_semantics_preserved_outside_declared_part_and_model_inputs(self) -> None:
        ...
        report = selection(self.root, self.project_id, self.draft(), self.receipt())
        manifest_edit = next(edit for edit in report.edits if edit.path == self.manifest_path.relative_to(self.root).as_posix())
    ...
    def test_selection_contract_rejects_schema_type_unknowns_and_missing_preconditions(self) -> None:
        selected = self.draft()
        original = selected.read_text()
        spec = read_model(selected, PartSelectionMap)
        ...
        selected.write_text(spec.model_copy(update={"preconditions": removed}).model_dump_json(), encoding="utf-8")
        result = selection(self.root, self.project_id, selected, self.receipt())
        self.assertEqual(result.status, "BLOCKED")
        for raw in (
            ...
            original.replace('"project_id": "controller"', '"project_id": "controller", "project_id": "controller"'),
        ):
            with self.subTest(raw=raw):
                selected.write_text(raw, encoding="utf-8")
                self.assertEqual(selection(self.root, self.project_id, selected, self.receipt()).status, "BLOCKED")
    ...
    def test_picker_disables_offboard_symbols_before_selection(self) -> None:
        self.schematic.write_bytes(self.schematic.read_bytes().replace(
    ...
    def replacement_draft(self, shared_old_part: bool) -> Path:
        ...
        for reference in references:
            source = source.replace(f'(property "Reference" "{reference}"',
        ...
        unrelated = self.part.model_copy(update={"id": "unrelated", "mpn": "UNRELATED-123", "cad": None})
        self.set_parts(self.part, old, unrelated)
        manifest = read_model(self.manifest_path, ProjectManifest)
        write_model(self.manifest_path, manifest.model_copy(update={
            "component_identity": ComponentIdentity(required=True, part_ids=("old-part", "unrelated")),
        }))
        write_model(self.preferences, PurchasingPreferences(boards=8, spare_percent=15, spare_minimum=4,
    ...
    def test_replacing_last_use_retires_only_its_manifest_id_and_supplier_override(self) -> None:
        ...
        assert preview.locked_map is not None
        result = selection(self.root, self.project_id, Path(preview.locked_map), self.receipt(), apply=True)
        ...
        prefs = read_model(self.preferences, PurchasingPreferences)
        self.assertEqual(prefs.digikey_skus, {"test-r1": "541-TEST-ND", "unrelated": "UNRELATED-ND"})
    ...
    def test_replacing_one_shared_use_preserves_old_id_and_supplier_for_unchanged_reference(self) -> None:
    ...
    def test_excluded_reference_still_preserves_its_part_declaration(self) -> None:
        ...
        self.schematic.write_text(source[:marker] + source[marker:].replace('(dnp no)', '(dnp yes)', 1), encoding="utf-8")
        from kicad_tooling.hwrepo.part_picker import _snapshot
        spec = read_model(selected, PartSelectionMap)
        selected.write_text(spec.model_copy(update={"preconditions": _snapshot(self.root, self.project_id)}).model_dump_json(), encoding="utf-8")
        preview = selection(self.root, self.project_id, selected, self.receipt())
        self.assertEqual(preview.status, "PLAN", preview.issues)
        manifest_edit = next(edit for edit in preview.edits if edit.path == self.manifest_path.relative_to(self.root).as_posix())
        self.assertIn("old-part", ProjectManifest.model_validate_json(manifest_edit.after).component_identity.part_ids)
