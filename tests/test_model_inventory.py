...
class ModelInventoryTests(unittest.TestCase):
    ...
    def test_unassigned_footprint_is_review_with_exact_stem_candidate(self) -> None:
        ...
        self.assertEqual(
            report.footprints[0].candidate_assets, ("projects/test-board/kicad/models/Part.step",)
        )
  ...
  (model "${KIPRJMOD}/models/Part.step" (offset (xyz 0 0 0)))
        ...
        self.assertEqual(
            ready.footprints[0].models[0].source_path, "projects/test-board/kicad/models/Part.step"
        )
    ...
    def test_hidden_only_model_and_malformed_board_are_explained(self) -> None:
        ...
        self.config = self.config.model_copy(
            update={
                "required_inputs": (
                    *self.config.required_inputs,
                    "projects/test-board/kicad/Part.step",
                ),
            }
        )
        self.write_board("""(footprint "Lib:Part" (property "Reference" "U1")
