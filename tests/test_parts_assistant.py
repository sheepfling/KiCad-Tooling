...
class FormTests(unittest.TestCase):
    def test_form_rejects_duplicate_unknown_and_invalid_quantities(self) -> None:
        ...
        for body in (
            b"boards=1",
            b"boards=1&spare_percent=0&spare_minimum=0&path=x",
    ...
    def test_form_converts_to_immutable_typed_values(self) -> None:
        self.assertEqual(
            Form.decode(b"part.R1=resistor-1k").assignments(),
            (PartSelectionAssignment(reference="R1", part_id="resistor-1k"),),
        )
        self.assertEqual(
            Form.decode(b"boards=4&spare_percent=10&spare_minimum=2").quantities(),
    ...
    def test_cad_source_form_accepts_only_exact_ids_and_bounded_expected_mpn(self) -> None:
        ...
        self.assertEqual(
            Form.decode(b"id=C2040&expected_mpn=NE555P").cad_source(), ("C2040", "NE555P")
        )
        for body in (
            "id=C2040",
            "id=C2040&expected_mpn=&path=/tmp/model",
            "id=C2040&expected_mpn=&refresh=true",
            "id=&expected_mpn=",
            "id=c2040&expected_mpn=",
            "id=C0&expected_mpn=",
            "id=C02040&expected_mpn=",
            "id=C../2040&expected_mpn=",
            "id=../C2040&expected_mpn=",
            "id=https://example.test/C2040&expected_mpn=",
            "id=C2040%0A&expected_mpn=",
            "id=" + "C" + "1" * 32 + "&expected_mpn=",
            "id=C2040&expected_mpn=%20NE555P",
            "id=C2040&expected_mpn=NE555P%0A",
            "id=C2040&expected_mpn=%7F",
    ...
    def test_page_has_no_source_html_injection_or_download_json_workflow(self) -> None:
        ...
        self.assertIn(
            "STEP export is not verified; review actual fit, dimensions and polarity.", html
        )
...
class AssistantHttpTests(unittest.TestCase):
    ...
    def test_cross_origin_and_non_form_writes_are_rejected_before_backend(self) -> None:
        with patch.object(Assistant, "execute") as execute:
            ...
            self.assertEqual(
                self.request("POST", "scan", origin="https://attacker.invalid")[0], 403
            )
    ...
    def cad_source_report(self) -> CadSourceReport:
        ...
        bundle = CadSourceBundle(
            ...
            symbol_file="parts.kicad_sym",
            symbol_name="TEST_1",
            footprint_file="parts.pretty/TEST_1.kicad_mod",
            footprint_name="TEST_1",
            ...
            issues=("Only a WRL model is available for this test part",),
        )
        return CadSourceReport(
            status="READY",
            supplier_id="C2040",
            bundle_directory=str(directory),
            bundle=bundle,
            receipt_directory=str(self.root / "build/provider-review"),
        )
    ...
    def cad_import_report(self, *, status: str = "PLAN") -> CadImportReport:
        return CadImportReport(
            ...
            symbol_id="Imported:TEST_1",
            footprint_id="Imported:TEST_1",
            files=("examples/projects/controller/kicad/sym-lib-table",),
            check=CadBundleCheck(
                status="READY",
                symbol_pins=("1", "2"),
                footprint_pads=("1", "2"),
                model_references=("${KIPRJMOD}/parts.3dshapes/test.wrl",),
            ),
            plan_path=str(self.root / "build/library-plan.json"),
            receipt_directory=str(self.root / "build/library-review"),
            diff="+ reviewed library registration\n",
        )

    def source_cad(
        self, *, source: CadSourceReport | None = None, report: CadImportReport | None = None
    ) -> tuple[int, dict[str, str], bytes]:
        with (
            patch(
                "kicad_tooling.hwrepo.cad_source.fetch",
                return_value=source or self.cad_source_report(),
            ),
            patch(
                "kicad_tooling.hwrepo.cad_library.plan",
                return_value=report or self.cad_import_report(),
            ),
        ):
    ...
    def test_cad_source_validates_form_before_any_provider_request(self) -> None:
        with (
            ...
            for body in (
                ...
                "id=../C2040&expected_mpn=",
                "id=c2040&expected_mpn=",
                "id=C2040&expected_mpn=&path=/private/secret",
                "id=C2040&expected_mpn=%0A",
            ):
    ...
    def test_cad_source_creates_fresh_typed_review_without_importing(self) -> None:
        ...
        with (
            patch("kicad_tooling.hwrepo.cad_source.fetch", return_value=source) as fetch,
            patch("kicad_tooling.hwrepo.cad_library.plan", return_value=planned) as plan,
        ...
        self.assertEqual(
            plan.call_args.args[:3], (self.root, "controller", Path(source.bundle_directory))
        )
    ...
    def test_step_review_serves_only_current_hash_checked_views(self) -> None:
        ...
        output.mkdir()
        files = {
            "index.html": b"<h1>Paired views</h1>",
            "index.css": b"body{color:black}",
            "wrl-top.png": b"WRL image",
            "step-top.png": b"STEP image",
        ...
        with patch("kicad_tooling.hwrepo.cad_step.review", return_value=result) as compare:
            ...
            self.assertEqual(
                compare.call_args.args[:3], (self.root, "controller", self.state.sourced_source)
            )
    ...
    def test_gallery_reads_wait_for_each_other_instead_of_dropping_images(self) -> None:
        ...
        with (
            patch.object(Assistant, "step_asset", side_effect=slow_read),
    ...
    def test_cad_source_can_omit_expected_mpn(self) -> None:
        with (
            patch(
                "kicad_tooling.hwrepo.cad_source.fetch", return_value=self.cad_source_report()
            ) as fetch,
            patch("kicad_tooling.hwrepo.cad_library.plan", return_value=self.cad_import_report()),
        ):
    ...
    def test_failed_cad_lookup_exposes_setup_guidance_and_clears_older_import(self) -> None:
        ...
        blocked = CadSourceReport(
            status="BLOCKED",
            supplier_id="C2040",
            receipt_directory="build/provider-review",
        ...
        with (
            ...
            self.assertEqual(
                self.request("POST", "import-cad", urlencode({"review": old_review}))[0], 400
            )
            plan.assert_not_called()
            apply.assert_not_called()
    ...
    def test_blocked_cad_plan_cannot_be_imported(self) -> None:
        planned = self.cad_import_report(status="BLOCKED").model_copy(
            update={"issues": ("Pin numbers differ",)}
        )
        ...
        with patch("kicad_tooling.hwrepo.cad_library.apply") as apply:
            self.assertEqual(
                self.request("POST", "import-cad", urlencode({"review": review["review_id"]}))[0],
                400,
            )
            apply.assert_not_called()
    ...
    def test_cad_import_requires_this_tabs_review_and_server_held_path(self) -> None:
        ...
        with patch(
            "kicad_tooling.hwrepo.cad_library.apply",
            return_value=self.cad_import_report(status="APPLIED"),
        ) as apply:
            for form in (
                {},
                {"review": first["review_id"]},
                {"review": "/private/secret"},
                {"path": str(planned_path)},
                {"review": second["review_id"], "path": "/private/secret"},
            ):
                with self.subTest(form=form):
                    self.assertEqual(self.request("POST", "import-cad", urlencode(form))[0], 400)
            apply.assert_not_called()
            status, _, body = self.request(
                "POST", "import-cad", urlencode({"review": second["review_id"]})
            )
            self.assertEqual(status, 200)
            self.assertEqual(json.loads(body)["status"], "APPLIED")
            self.assertEqual(apply.call_args.args[:3], (self.root, "controller", planned_path))
            self.assertEqual(
                self.request("POST", "import-cad", urlencode({"review": second["review_id"]}))[0],
                400,
            )
    ...
    def test_failed_import_consumes_plan_and_does_not_retry_on_repeat(self) -> None:
        ...
        with patch(
            "kicad_tooling.hwrepo.cad_library.apply",
            side_effect=ValueError("Source changed since review"),
        ) as apply:
    ...
    def test_other_source_apply_invalidates_pending_cad_import(self) -> None:
        review = json.loads(self.source_cad()[2])
        self.state.cad_plan = self.root / "build/old-cad.json"
        applied = AutoCadReport(
            project_id="controller", status="APPLIED", receipt_directory="build/applied"
        )
        with (
            patch("kicad_tooling.hwrepo.auto_cad.apply", return_value=applied),
            patch("kicad_tooling.hwrepo.cad_library.apply") as apply,
        ):
    ...
    def test_cad_scan_and_apply_use_only_the_server_held_plan(self) -> None:
        ...
        report = AutoCadReport(
            project_id="controller",
            status="PLAN",
            plan_path=str(plan_path),
            receipt_directory=str(self.root / "build"),
            files=("examples/projects/controller/kicad/model.step",),
            items=(
                AutoCadItem(
                    reference="R1", footprint="Device:R", status="READY", detail="Matched pair"
                ),
            ),
        )
    ...
    def test_partial_cad_review_still_has_exact_diff_and_per_reference_details(self) -> None:
        report = AutoCadReport(
            ...
            receipt_directory="build/review",
            diff="- old\n+ <script>untrusted</script>\n",
            items=(
                AutoCadItem(
                    reference="D1",
                    footprint="LED:Custom",
                    status="NEEDS_REVIEW",
    ...
    def test_picker_selection_cannot_submit_unoffered_identity_or_source_hashes(self) -> None:
        component = PartCadComponent(
            ...
            footprint="Device:R",
            symbol_id="Device:R",
            source_path="examples/projects/controller/kicad/controller.kicad_sch",
            uuid="uuid",
        )
        self.state.picker = PartPickerReport(
            project_id="controller",
            status="READY",
            receipt_dir="build/picker",
            items=(PartPickerItem(component=component, choice_ids=("resistor-1k",)),),
            selection_template=PartSelectionMap(
                project_id="controller", preconditions={"catalog/parts.json": "1" * 64}
            ),
        )
        ...
        report = PartSelectionReport(
            project_id="controller",
            status="PLAN",
            receipt_dir="build/review",
            locked_map=str(locked),
        )
        with patch(
            "kicad_tooling.hwrepo.parts_assistant.selection", return_value=report
        ) as selecting:
            self.assertEqual(self.request("POST", "select", "part.R1=unoffered")[0], 400)
            self.assertEqual(
                self.request("POST", "select", "part.R1=resistor-1k&preconditions=fake")[0], 400
            )
    ...
    def test_order_download_cannot_leak_other_files_and_rejects_stale_sources(self) -> None:
        ...
        report = PurchasingReport(
            ...
            input_hashes=input_hashes(self.root, "controller", None),
            artifacts=("bom.csv",),
        )
    ...
    def test_tampered_download_is_rejected(self) -> None:
        ...
        _, _, source_hashes = project_context(self.root, "controller")
        self.state.order = PurchasingReport(
            project_id="controller",
            status="NEEDS_PARTS",
            receipt_dir=str(receipt),
            source_hashes=source_hashes,
    ...
    def ready_order(self) -> PurchasingReport:
        ...
        csv.write_text(
            "PartNumber,Quantity,CustomerReference\nRC0603FR-071KL,8,RES-1\n", encoding="utf-8"
        )
        _, _, source_hashes = project_context(self.root, "controller")
        plan = PurchasingPlan(
            ...
            lines=(
                PurchasingLine(
                    ...
                    spares=2,
                    quantity=8,
                    order_number="RC0603FR-071KL",
                    order_number_kind="MPN",
    ...
    def handoff(self, review: str | None = None):
        return self.request(
            "POST", "digikey", urlencode({"review": review or str(self.root / "build/order")})
        )
    ...
    def test_handoff_requires_ready_server_plan_and_exact_review_field(self) -> None:
        with patch("kicad_tooling.hwrepo.digikey_handoff.send") as send:
            ...
            for body in (
                ...
                "review=",
                "review=%20",
                "quantity=999&part=arbitrary",
                "review=old&quantity=999",
            ):
    ...
    def test_new_preparation_rejects_other_tabs_old_review_before_send_or_cache(self) -> None:
            ...
            updated = plan.model_copy(
                update={
                    "preferences": PurchasingPreferences(
                        boards=boards, spare_percent=spare_percent, spare_minimum=spare_minimum
                    ),
                    "lines": (
                        plan.lines[0].model_copy(
                            update={"required": boards * 2, "spares": 0, "quantity": boards * 2}
                        ),
                    ),
                }
            )
        ...
        with patch("kicad_tooling.hwrepo.parts_assistant.prepare", side_effect=projected):
            first = json.loads(
                self.request("POST", "order", "boards=1&spare_percent=0&spare_minimum=0")[2]
            )
            second = json.loads(
                self.request("POST", "order", "boards=10&spare_percent=0&spare_minimum=0")[2]
            )
    ...
    def test_handoff_network_failure_preserves_csv_and_never_implicitly_retries(self) -> None:
        report = self.ready_order()
        with patch(
            "kicad_tooling.hwrepo.digikey_handoff.send", side_effect=OSError("Connection lost")
        ) as send:
            status, _, body = self.handoff()
            self.assertEqual(status, 200)
            ...
            send.assert_called_once()
            with (
                patch("kicad_tooling.hwrepo.parts_assistant.prepare", return_value=report),
                patch("kicad_tooling.hwrepo.parts_assistant.save_report"),
            ):
                self.assertEqual(
                    self.request("POST", "order", "boards=3&spare_percent=0&spare_minimum=0")[0],
                    200,
                )
    ...
    def test_order_form_passes_only_validated_quantities(self) -> None:
        report = PurchasingReport(
            project_id="controller",
            status="BLOCKED",
            receipt_dir="build/unused",
            issues=("Test capture unavailable",),
        )
        with patch.object(Assistant, "prepare_order", return_value=report) as prepare:
            status, _, _ = self.request(
                "POST", "order", urlencode({"boards": 5, "spare_percent": 10, "spare_minimum": 2})
            )
        self.assertEqual(status, 200)
        self.assertEqual(
            prepare.call_args.args[0],
            PurchasingPreferences(boards=5, spare_percent=10, spare_minimum=2),
        )
