...
FOOTPRINT = (
    f'(footprint "TwoPin" (layer "F.Cu") {PADS} (model "${{KIPRJMOD}}/{MODEL_PATH}" {TRANSFORM}))\n'
)
...
class CadLibraryTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        for path, content in (
            (self.symbol_path, SYMBOL.encode()),
            (self.footprint_path, FOOTPRINT.encode()),
            (self.model_path, WRL),
        ):
        ...
        self.bundle = CadSourceBundle(
            ...
            symbol_name="TwoPin",
            footprint_file="library/part.pretty/TwoPin.kicad_mod",
            footprint_name="TwoPin",
    ...
    def test_imports_complete_native_library_without_touching_design_or_catalog(self) -> None:
        protected = {
            ...
            for path in (
                self.island / "kicad/controller.kicad_sch",
                self.island / "kicad/controller.kicad_pcb",
        ...
        self.assertTrue(
            pair.models[0].source_model_reference.startswith("${KIPRJMOD}/cad/sourced/")
        )
        ...
        for name in report.files:
            if name != "examples/projects/controller/project.json":
                self.assertIn(
                    Path(name).relative_to("examples/projects/controller").as_posix(),
    ...
    def test_mismatched_identity_blocks_without_source_writes(self) -> None:
        for old, new in (
            ("Test Fixture", "Different Manufacturer"),
            ("TEST-ONLY-1", "OTHER-MPN"),
            ("C1234", "C9999"),
            ("part:TwoPin", "part:OtherFootprint"),
        ):
    ...
    def test_stacked_pins_repeated_pads_and_unplated_mechanical_holes_are_supported(self) -> None:
        ...
        extra = (
            '(pad "1" smd rect (at 0 4) (size 1 1) (layers "F.Cu" "F.Paste" "F.Mask"))'
    ...
    def test_missing_hidden_or_escaping_model_is_blocked(self) -> None:
        variants = (
            FOOTPRINT.replace(f'(model "${{KIPRJMOD}}/{MODEL_PATH}" {TRANSFORM})', ""),
        ...
        for source in variants:
            with self.subTest(source=source):
                ...
                self.save_bundle()
                self.assertEqual(
                    cad_library.inspect_bundle(self.bundle_dir, self.bundle).status, "BLOCKED"
                )
    ...
    def test_duplicate_manifest_paths_cannot_silently_overwrite_hashes(self) -> None:
        self.bundle = self.bundle.model_copy(
            update={"files": (*self.bundle.files, self.bundle.files[0])}
        )
        write_model(self.bundle_dir / "bundle.json", self.bundle)
        checked = cad_library.inspect_bundle(self.bundle_dir, self.bundle)
    ...
    def test_unsafe_manifest_paths_and_symlinked_assets_are_rejected(self) -> None:
        for unsafe in ("../outside.wrl", "/tmp/model.wrl", "C:/model.wrl", "library\\model.wrl"):
            with self.subTest(path=unsafe):
                ...
                self.assertEqual(
                    cad_library.inspect_bundle(self.bundle_dir, altered).status, "BLOCKED"
                )
        write_model(self.bundle_dir / "bundle.json", self.bundle)
    ...
    def test_case_conflicting_manifest_paths_are_rejected_on_all_hosts(self) -> None:
        alias = CadSourceFile(
            path=self.bundle.files[0].path.upper(), sha256=self.bundle.files[0].sha256
        )
    ...
    def test_nickname_collision_is_not_overwritten(self) -> None:
        ...
        table.write_text(
            before[: before.rfind(")")] + f'(lib (name "{nickname}")(type "KiCad")'
    ...
    def test_mid_transaction_failure_restores_every_original_file(self) -> None:
        ...
        self.assertEqual(
            {path: path.read_bytes() for path in self.island.rglob("*") if path.is_file()}, before
        )
    ...
    def test_strict_contracts_round_trip_and_reject_unknown_or_wrong_fields(self) -> None:
        ...
        self.assertEqual(
            CadSourceBundle.model_validate_json(self.bundle.model_dump_json()), self.bundle
        )
        assert report.plan_path is not None
