...
class ContractCoachTests(unittest.TestCase):
        ...
        self.summary = ValidationSummary(
            ...
            checks={
                ...
                "netlist": CheckEvidence(
                    status="FAIL", returncode=0, error="Independent contract differs"
                ),
            },
            status="FAIL",
            artifacts_sha256={
        ...
        self.assertEqual(
            {
                ...
                for component in report.differences
    ...
    def test_capture_rejects_source_changed_during_export(self) -> None:
        ...
        output = receipt_directory(self.root, self.project_id, None)
        report = capture(
            self.root, self.project_id, output, MutatingRunner(self.config.kicad_version)
        )
        self.assertEqual(report.status, "BLOCKED")
    ...
    def test_container_runner_uses_pinned_image_and_read_only_source(self) -> None:
        ...
        self.assertIn(("-v", f"{output.parent}:/output:rw"), tuple(zip(calls[1], calls[1][1:])))
        self.assertEqual(calls[1][calls[1].index("--output") + 1], "/output/netlist.xml")
        self.assertEqual(
            calls[1][-1], "/work/examples/projects/controller/kicad/controller.kicad_sch"
        )
    ...
    def test_windows_container_resolves_docker_command_extension(self) -> None:
        with (
            ...
            patch(
                "kicad_tooling.hwrepo.contract_coach.shutil.which", return_value="C:/bin/docker.cmd"
            ),
        ):
    ...
    def test_auto_fallback_capture_retains_both_probes_in_ignored_receipt(self) -> None:
        ...
        self.assertEqual(
            set(report.commands), {"local_version", "container_version", "version", "netlist"}
        )
    ...
    def test_cli_capture_runs_local_adapter_and_retains_command_evidence(self) -> None:
        executable = fake_executable(
            self.root / "build/fake-kicad-cli",
            "import pathlib, sys\n"
