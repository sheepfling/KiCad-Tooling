...
class PickerViewTests(unittest.TestCase):
    def picker(self) -> PartPickerReport:
        part = PartRecord(
            id='reviewed-r', revision='A', description='Example <em>untrusted</em>',
            part_class='resistor', unit='each', manufacturer='Test & Co', mpn='FIXTURE-1K',
            datasheet_url='https://example.invalid', lifecycle='fixture', status=PartStatus.APPROVED,
            cad=PartCadBinding(symbol_id='Test:R', value='1k', footprint='Test:R',
                               model='projects/pilot/kicad/body.step', digikey_sku='FIXTURE-SKU'),
        )
        component = PartCadComponent(reference='R1', symbol_id='Test:R', value='1k',
            footprint='Test:R', source_path='projects/pilot/kicad/pilot.kicad_sch', uuid='fixture')
        return PartPickerReport(status='READY', project_id='pilot', receipt_dir='/tmp/receipt',
            items=(PartPickerItem(component=component, choice_ids=(part.id,)),), choices=(part,),
            selection_template=PartSelectionMap(project_id='pilot',
                preconditions={'projects/pilot/docs/purchasing.json': None,
    ...
    def test_missing_catalog_has_action_and_no_invented_choice(self) -> None:
        report = self.picker().model_copy(update={'status': 'NEEDS_CATALOG', 'choices': (),
    ...
    def test_blocked_picker_does_not_offer_download(self) -> None:
        report = self.picker().model_copy(update={'status': 'BLOCKED',
            'selection_template': None, 'issues': ('Source changed; create a fresh picker.',)})
        page = render_picker(report)
    ...
    def test_preview_escapes_diff_and_reports_placement_gap(self) -> None:
        report = PartSelectionReport(status='PLAN', project_id='pilot', receipt_dir='/tmp/receipt',
            edits=(PartSourceEdit(path='projects/pilot/kicad/pilot.kicad_sch',
                                 before='(property "Value" "<script>")\n',
                                 after='(property "Value" "<em>")\n'),),
            pending_references=('R1',), next_commands=('python -B -m kicad_tooling.parts --sync-models',))
    ...
    def test_conflicting_cli_flags_fail_before_creating_receipt(self) -> None:
        ...
        cases = (('--apply',), ('--picker', '--boards', '5'),
                 ('--sync-models', '--apply'), ('--selection', 'x.json', '--native-summary', 'x'),
                 ('--picker', '--init-preferences', 'x.json'),
                 ('--selection', 'x.json', '--runner', 'container'))
        for arguments in cases:
            with self.subTest(arguments=arguments):
                result = subprocess.run((sys.executable, '-B', '-m', 'kicad_tooling.parts',
                    '--project', 'invalid-no-discovery', *arguments), cwd=root,
