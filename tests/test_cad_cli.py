...
class CadCliTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.bundle = CadSourceBundle(
            ...
            symbol_name="RP2040",
            footprint_file="library/part.pretty/QFN.kicad_mod",
            footprint_name="QFN",
            model_file="library/part.3dshapes/QFN.wrl",
            files=(
                CadSourceFile(path="library/part.kicad_sym", sha256="a" * 64),
        ...
        self.source = CadSourceReport(
            ...
            bundle=self.bundle,
            receipt_directory=str(self.receipt),
        )
        self.planned = CadImportReport(
            status="PLAN",
            project_id="controller",
            symbol_id="CAD_C2040:RP2040",
            footprint_id="CAD_C2040:QFN",
    ...
    def test_source_json_passes_exact_identity_refresh_and_receipts_without_applying(self) -> None:
        with (
            patch(
                "kicad_tooling.parts.new_receipt", side_effect=[self.receipt, self.plan_receipt]
            ) as receipts,
        ...
        fetch.assert_called_once_with(
            self.root, "C2040", self.receipt, expected_mpn="RP2040", refresh=True
        )
        plan.assert_called_once_with(
            self.root, "controller", self.bundle_directory, self.plan_receipt
        )
        self.assertEqual(
            receipts.call_args_list[0].args, (self.root, "controller", Path("build/chosen"))
        )
    ...
    def test_text_followup_preserves_custom_root_and_quotes_every_argument(self) -> None:
        ...
        command = next(
            line.removeprefix("Then: ") for line in stdout.splitlines() if line.startswith("Then: ")
        )
        self.assertEqual(
            split(command),
            [
    ...
    def test_check_step_uses_exact_source_and_reports_review_or_block(self) -> None:
        for ready in (True, False):
            ...
            with (
                ...
                status, stdout = self.invoke(
                    ["--check-step", "C2040", "--expected-mpn", "RP2040", "--format", "json"]
                )
            self.assertEqual(status, 0 if ready else 1)
            self.assertEqual(CadStepReport.model_validate_json(stdout), result)
            fetch.assert_called_once_with(
                self.root, "C2040", self.plan_receipt, expected_mpn="RP2040", refresh=False
            )
    ...
    def test_import_uses_only_locked_plan_and_reports_applied_or_blocked_status(self) -> None:
        for applied in (True, False):
            report = CadImportReport(
                ...
                symbol_id="CAD_C2040:RP2040",
                receipt_directory=str(self.receipt),
                issues=() if applied else ("Source changed",),
            )
            with (
                self.subTest(applied=applied),
                patch("kicad_tooling.parts.new_receipt", return_value=self.receipt),
                ...
                status, stdout = self.invoke(
                    ["--import-cad", str(self.plan_path), "--apply", "--format", "json"]
                )
