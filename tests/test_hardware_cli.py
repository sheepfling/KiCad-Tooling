...
class HardwareCliTests(unittest.TestCase):
    ...
    def test_check_text_explains_generation_drift(self) -> None:
        ...
        with (
            patch("kicad_tooling.hardware.check", return_value=report),
            patch("kicad_tooling.hardware.check_generation", return_value=("STALE_OUTPUT: old.json",)),
            patch.object(sys, "argv", ["kicad_tooling.hardware", "check", "--format", "text"]),
            patch("sys.stdout", new_callable=StringIO) as output,
        ):
            self.assertEqual(main(), 1)
    ...
    def test_generate_text_summarizes_files_and_error_action(self) -> None:
        with (
            patch("kicad_tooling.hardware.generate", return_value=("generated/one.json", "schemas/two.json")),
    ...
    def test_snapshot_and_verification_text_state_review_scope(self) -> None:
        ...
        with (
            ...
            patch.object(sys, "argv", ["kicad_tooling.hardware", "snapshot", "--output", "build/review-1", "--format", "text"]),
            patch("sys.stdout", new_callable=StringIO) as output,
        ):
            self.assertEqual(main(), 0)
        text = output.getvalue()
        ...
        with (
            ...
            patch.object(sys, "argv", ["kicad_tooling.hardware", "verify-snapshot", "--output", "build/review-1", "--format", "text"]),
            patch("sys.stdout", new_callable=StringIO) as output,
        ):
            self.assertEqual(main(), 0)
