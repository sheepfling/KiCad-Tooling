...
class ContainerGitTests(unittest.TestCase):
    def setUp(self) -> None:
        ...
        self.git(self.root, "-c", "user.name=Test fixture", "-c", "user.email=fixture@example.invalid",
    ...
    def environment(arguments: tuple[str, ...]) -> dict[str, str]:
        return dict(arguments[index + 1].split("=", 1)
                    for index, value in enumerate(arguments) if value == "-e")
    ...
    def test_standard_clone_does_not_add_mounts_or_environment(self) -> None:
        ...
        command = container_command(self.root, "synthetic-image", "controller",
    ...
    def test_linked_metadata_mount_preserves_common_directory_relationship(self) -> None:
        ...
        (self.worktree / "README.md").write_text("Distinct linked-worktree source\n", encoding="utf-8")
        self.git(self.worktree, "-c", "user.name=Test fixture", "-c",
                 "user.email=fixture@example.invalid", "commit", "-qam", "Linked-only change")
        ...
        self.assertIn(f"type=bind,source={common.as_posix()},target={CONTAINER_COMMON},readonly", arguments)
        environment = self.environment(arguments)
        self.assertEqual(environment, {
            "GIT_DIR": expected_git, "GIT_COMMON_DIR": CONTAINER_COMMON,
            "GIT_WORK_TREE": "/work", "GIT_OPTIONAL_LOCKS": "0",
        })
        ...
        host_environment = {**os.environ, **environment,
                            "GIT_DIR": str(git_dir), "GIT_COMMON_DIR": str(common),
                            "GIT_WORK_TREE": str(self.worktree)}
        commit = subprocess.run(("git", "rev-parse", "HEAD"), cwd=self.worktree,
                                env=host_environment, text=True, capture_output=True, check=True).stdout.strip()
        self.assertEqual(commit, selected_commit)
        self.assertNotEqual(commit, main_commit)
        files = subprocess.run(("git", "ls-files"), cwd=self.worktree, env=host_environment,
    ...
    def test_verify_command_includes_worktree_mapping_before_image(self) -> None:
        ...
        command = container_command(self.worktree, image, "controller",
                                    self.worktree / "build/deps", self.worktree / "build/native")
    ...
    def test_release_validation_and_export_share_the_worktree_mapping(self) -> None:
        ...
        project = next(item for item in load_registry(reference).projects if item.id == "controller")
        ...
        def execute(argv, **kwargs):
            if argv[0] == "docker":
                ...
                return subprocess.CompletedProcess(argv, 0, stdout="Synthetic native result", stderr="")
            return real_run(argv, **kwargs)

        for export_only in (False, True):
            with (patch("kicad_tooling.hwrepo.releasing.load_config", return_value=config),
                  patch("kicad_tooling.hwrepo.releasing.subprocess.run", side_effect=execute)):
                releasing.run_native(self.worktree, project,
                                     self.worktree / f"build/run-{export_only}/native", None,
                                     Path("build/deps"), export_only=export_only)
        self.assertEqual(len(commands), 2)
        for command in commands:
            self.assertEqual(self.environment(command)["GIT_COMMON_DIR"], CONTAINER_COMMON)
            self.assertTrue(any(value.endswith(f"target={CONTAINER_COMMON},readonly") for value in command))
    ...
    def test_unsupported_git_directory_relationship_is_rejected(self) -> None:
        ...
        output = f"{git_dir}\n{self.root / '.git'}\n{self.worktree}\n"
        with (patch("kicad_tooling.hwrepo.container_git.subprocess.run", return_value=
    ...
    def test_unsafe_mount_delimiters_and_wrong_root_are_rejected(self) -> None:
        ...
        for output, message in (
            ...
            with (self.subTest(message=message),
                  patch("kicad_tooling.hwrepo.container_git.subprocess.run", return_value=
                        subprocess.CompletedProcess([], 0, stdout=output)),
