...
class HostedPreviewTests(unittest.TestCase):
    ...
    def child(self, code: int, *, report: bool = True):
        ...
        def launch(log: HostedLog, stage: str, argv: tuple[str, ...], *, cwd: Path) -> Path:
            ...
            return run(log, stage, (sys.executable, "-I", "-X", "utf8", "-c", script, output), cwd=cwd)

        return launch

    def test_success_retains_unicode_logs_and_escapes_summary(self) -> None:
        self.summary.write_text("Earlier stage\n", encoding="utf-8")
        with (patch.object(HostedLog, "run", self.child(0)),
              redirect_stdout(StringIO()) as stdout, redirect_stderr(StringIO())):
            preview_lane(self.root, "controller", HostedLog(self.root, "preview"),
                         output=self.output, cli="C:/Program Files/KiCad/bin/kicad-cli.exe")
        self.assertIn("preview café", stdout.getvalue())
        self.assertIn("café", (self.output / "cli.stdout.txt").read_text(encoding="utf-8"))
        self.assertIn("diagnostic café", (self.output / "cli.stderr.txt").read_text(encoding="utf-8"))
    ...
    def test_failed_renderer_preserves_original_exit_and_partial_report(self) -> None:
        with (patch.object(HostedLog, "run", self.child(7)),
              patch.object(sys, "argv", ["ci-hosted", "--root", str(self.root), "preview",
                                       "--project", "controller", "--output", str(self.output)]),
              redirect_stdout(StringIO()), redirect_stderr(StringIO())):
            self.assertEqual(main(), 7)
        self.assertTrue((self.output / "visualization.txt").is_file())
        events = [json.loads(line) for line in
                  (self.root / "build/ci-hosted/preview/events.jsonl").read_text().splitlines()]
    ...
    def test_early_failure_still_retains_logs_and_explains_missing_report(self) -> None:
        with (patch.object(HostedLog, "run", self.child(2, report=False)),
              redirect_stdout(StringIO()), redirect_stderr(StringIO()),
              self.assertRaises(HostedCommandError)):
            preview_lane(self.root, "controller", HostedLog(self.root, "preview"), output=self.output)
    ...
    def test_existing_output_is_never_overwritten_or_published_as_fresh(self) -> None:
        ...
        with (patch.object(HostedLog, "run") as run, redirect_stderr(StringIO()),
    ...
    def test_source_or_external_output_is_rejected(self) -> None:
        for output in (self.root / "docs", self.root.parent / "elsewhere",
                       self.root / "build/../docs", self.root / "build"):
            with (self.subTest(output=output), redirect_stderr(StringIO()),
                  self.assertRaises(ValueError)):
                preview_lane(self.root, "controller", HostedLog(self.root, "preview"), output=output)
    ...
    def test_actions_project_environment_and_local_runner_are_forwarded(self) -> None:
        with (patch.dict(os.environ, {"PROJECT_ID": "controller"}),
              patch.object(sys, "argv", ["ci-hosted", "--root", str(self.root), "preview",
                                       "--runner", "local"]),
