...
class PartsExtensionsTests(unittest.IsolatedAsyncioTestCase):
    ...
    def use_order(self) -> PurchasingReport:
        ...
        self.order_path = (
            (Path(report.receipt_dir) / "report.json").relative_to(self.root).as_posix()
        )
        return report
            ...
            ValidationSummary(
                ...
                checks={
                    ...
                    "netlist": CheckEvidence(
                        status="FAIL", returncode=0, error="Synthetic contract mismatch"
                    ),
                },
                status="FAIL",
                artifacts_sha256={
    ...
    def sources(self):
        return {
            ...
            if path.is_file() and path.relative_to(self.root).parts[0] != "build"
        }
    ...
    async def test_picker_selection_and_sync_cli_protocol_parity(self) -> None:
        ...
        async with Client(server) as client:
            ...
            self.assertEqual(
                self.semantic(cli_picker, cli_picker.receipt_dir),
            ...
            before = self.sources()
            cli_preview = await self.cli(
                PartSelectionReport,
                "--selection",
                str(draft),
                "--output",
                "build/parts/cli-selection",
            )
            preview = await self.call(
                client,
                "preview_part_selection",
                PartSelectionReport,
                project_id="controller",
                view_id="mcp-selection",
            ...
            self.assertEqual(
                self.semantic(cli_applied, cli_applied.receipt_dir),
                self.semantic(applied, applied.receipt_dir),
            )
            self.assertEqual(self.sources(), cli_sources)
            cli_sync = await self.cli(
                PartSelectionReport, "--sync-models", "--output", "build/parts/cli-sync"
            )
            sync = await self.call(
                client,
                "preview_model_sync",
                PartSelectionReport,
                project_id="controller",
            ...
            self.assertEqual(
                self.semantic(cli_sync, cli_sync.receipt_dir), self.semantic(sync, sync.receipt_dir)
            )
    ...
    async def test_picker_and_edit_capabilities_and_stale_evidence(self) -> None:
        ...
        async with Client(create_server(self.root)) as client:
            ...
            self.assertFalse(
                {"prepare_part_picker", "apply_part_selection", "submit_supplier_handoff"} & names
            )
        async with Client(create_server(self.root, allow_exports=True)) as client:
            with patch("kicad_tooling.hwrepo.contract_coach.capture") as capture:
                refused = await client.call_tool(
                    "prepare_part_picker", {"project_id": "controller", "view_id": "no-native"}
                )
            self.assertTrue(refused.is_error)
            capture.assert_not_called()
            self.picker_fixture.schematic.write_bytes(
                self.picker_fixture.schematic.read_bytes() + b"\n"
            )
            cli = await self.cli(
                PartPickerReport,
                "--picker",
                "--native-summary",
                "build/native/controller/summary.json",
                "--output",
                "build/parts/cli-stale",
                expected_exit=1,
            )
            ...
            self.assertEqual(
                self.semantic(cli, cli.receipt_dir), self.semantic(stale, stale.receipt_dir)
            )
    ...
    async def test_auto_cad_cli_protocol_parity(self) -> None:
        ...
        with patch.dict(
            os.environ,
            {
                "KICAD10_FOOTPRINT_DIR": str(member.parent.parent),
                "KICAD10_3DMODEL_DIR": str(model.parent.parent),
            },
        ):
            cli_preview = await self.cli(
                AutoCadReport, "--auto-models", "--output", "build/parts/cli-cad"
            )
            async with Client(
                create_server(self.root, allow_exports=True, allow_edits=True)
            ) as client:
                preview = await self.call(
                    client,
                    "preview_auto_cad",
                ...
                self.assertEqual(preview.status, "PLAN", preview.issues)
                self.assertEqual(
                    self.semantic(cli_preview, cli_preview.receipt_directory),
                ...
                cli_applied = await self.cli(
                    AutoCadReport,
                    "--cad-plan",
                    cli_preview.plan_path,
                    "--apply",
                    "--output",
                    "build/parts/cli-cad-apply",
                )
                ...
                with patch(
                    "kicad_tooling.hwrepo.mcp_parts_extensions._output",
                    ...
                    applied = await self.call(
                        ...
                        AutoCadReport,
                        project_id="controller",
                        view_id="mcp-cad-apply",
                        plan=reviewed_path.relative_to(self.root).as_posix(),
                        expected_sha256=reviewed_sha,
                    )
                self.assertEqual(applied.status, "APPLIED", applied.issues)
                self.assertEqual(
                    self.semantic(cli_applied, cli_applied.receipt_directory),
                ...
                stale = await client.call_tool(
                    "apply_auto_cad",
                    {
                        "project_id": "controller",
                        "view_id": "bad-digest",
                        "plan": Path(preview.plan_path).relative_to(self.root).as_posix(),
    ...
    async def test_download_permission_and_cache_symlink_boundaries(self) -> None:
        ...
        with patch("kicad_tooling.hwrepo.cad_download._download") as transfer:
            with self.assertRaisesRegex(ValueError, "allow-downloads"):
                fetch_official_footprint(
                    cache, "Library:Exact_Name", "10.0.5", allow_downloads=False
                )
        ...
        version_cache.mkdir()
        (version_cache / "kicad-10.0.5").symlink_to(
            cache / "kicad-10.0.5", target_is_directory=True
        )
        with patch("kicad_tooling.hwrepo.cad_download._download") as transfer:
            with self.assertRaisesRegex(ValueError, "symlink"):
                fetch_official_footprint(
                    version_cache, "Library:Exact_Name", "10.0.5", allow_downloads=False
                )
            transfer.assert_not_called()
        pcb = self.root / "examples/projects/controller/kicad/controller.kicad_pcb"
    ...
    async def test_supplier_prepare_and_submit_cli_protocol_parity(self) -> None:
        ...
        with patch(
            "kicad_tooling.hwrepo.digikey_handoff._post",
            side_effect=AssertionError("offline preparation"),
        ):
            cli = await self.cli(
                SupplierHandoffReport,
                "--prepare-handoff",
        ...
        self.assertEqual(
            digest((self.root / prepared.handoff).parent / "payload.json"), prepared.payload_sha256
        )
        self.assertEqual(
            plan.payload_sha256,
            hashlib.sha256(plan.payload.model_dump_json(by_alias=True).encode()).hexdigest(),
        )
        ...
        with patch(
            "kicad_tooling.hwrepo.digikey_handoff._post",
            return_value=json.dumps(_REVIEW_URL).encode(),
        ) as post:
            async with Client(create_server(self.root, allow_supplier_submissions=True)) as client:
                result = await self.call(
                    client,
                    "submit_supplier_handoff",
                    SupplierHandoffReport,
                    handoff=prepared.handoff,
                    expected_sha256=prepared.handoff_sha256,
                )
            self.assertEqual(post.call_count, 1)
        self.assertEqual(
            self.semantic(result, result.handoff), self.semantic(cli_result, cli_result.handoff)
        )
        # A copied handle in the same checkout shares the durable attempt digest.
        with patch(
            "kicad_tooling.hwrepo.digikey_handoff._post",
            side_effect=AssertionError("must not resubmit"),
        ):
            async with Client(create_server(self.root, allow_supplier_submissions=True)) as client:
                repeated = await self.call(
                    ...
                    SupplierHandoffReport,
                    handoff=cli.handoff,
                    expected_sha256=cli.handoff_sha256,
                )
        self.assertEqual(repeated.handoff, cli.handoff)
        self.assertEqual(
            self.semantic(repeated, repeated.handoff), self.semantic(result, result.handoff)
        )
    ...
    async def test_supplier_capability_digest_and_stale_review_reject_before_network(self) -> None:
        ...
        async with Client(
            create_server(self.root, allow_exports=True, allow_edits=True, allow_checks=True)
        ) as client:
            self.assertNotIn(
                "submit_supplier_handoff", {tool.name for tool in (await client.list_tools()).tools}
            )
        ...
        with patch("kicad_tooling.hwrepo.digikey_handoff._post") as post:
            async with Client(create_server(self.root, allow_supplier_submissions=True)) as client:
                self.assertTrue(
                    (
                        await client.call_tool(
                            "submit_supplier_handoff", {**arguments, "expected_sha256": "0" * 64}
                        )
                    ).is_error
                )
                ...
                self.assertTrue(
                    (await client.call_tool("submit_supplier_handoff", arguments)).is_error
                )
    ...
    async def test_supplier_uncertain_attempt_is_persistent_and_never_retried(self) -> None:
        ...
        with patch(
            "kicad_tooling.hwrepo.digikey_handoff._post",
            side_effect=OSError("synthetic disconnect"),
        ) as post:
            async with Client(create_server(self.root, allow_supplier_submissions=True)) as client:
                result = await self.call(
                    client, "submit_supplier_handoff", SupplierHandoffReport, **arguments
                )
            self.assertEqual(post.call_count, 1)
        self.assertEqual(result.status, "UNCERTAIN")
        assert result.attempt_receipt is not None
        self.assertEqual(
            read_model(self.root / result.attempt_receipt, SupplierHandoffReport), result
        )
        with patch(
            "kicad_tooling.hwrepo.digikey_handoff._post",
            side_effect=AssertionError("must never retry"),
        ) as post:
            async with Client(create_server(self.root, allow_supplier_submissions=True)) as client:
    ...
    async def test_supplier_attempt_is_reserved_before_post_and_source_drift_remains_blocked(
        self,
    ) -> None:
        self.use_order()
        prepared = await self.prepared_handoff()
        ...
        def external_reply(path, body):
            reserved.append(
                supplier_handoff.submit_supplier_handoff(
        ...
        with patch(
            "kicad_tooling.hwrepo.digikey_handoff._post", side_effect=external_reply
        ) as post:
            async with Client(create_server(self.root, allow_supplier_submissions=True)) as client:
                result = await self.call(
                    client,
    ...
    async def test_part_apply_uses_reviewed_bytes_when_original_map_is_swapped(self) -> None:
        ...
        alternate = fixture.part.model_copy(
            update={"id": "test-r1-alternate", "mpn": "TEST-ALTERNATE"}
        )
        ...
        write_model(
            ...
            spec.model_copy(
                update={
                    "assignments": (PartSelectionAssignment(reference="R1", part_id=alternate.id),)
                }
            ),
        )
        ...
        with patch(
            "kicad_tooling.hwrepo.mcp_parts_extensions._output", side_effect=swap_after_review
        ):
            async with Client(create_server(self.root, allow_edits=True)) as client:
                applied = await self.call(
                    ...
                    project_id="controller",
                    view_id="swap-apply",
                    selection_map=original.relative_to(self.root).as_posix(),
    ...
    async def test_supplier_digest_race_cannot_substitute_another_valid_payload(self) -> None:
        ...
        self.order_path = (
            (Path(second_order.receipt_dir) / "report.json").relative_to(self.root).as_posix()
        )
        ...
        with (
            patch("kicad_tooling.hwrepo.supplier_handoff.digest", side_effect=swap_around_digest),
            patch(
                "kicad_tooling.hwrepo.digikey_handoff._post",
                return_value=json.dumps(_REVIEW_URL).encode(),
            ) as post,
        ...
        for call in post.call_args_list:
            self.assertEqual(
                call.args[1], first_spec.payload.model_dump_json(by_alias=True).encode()
            )
        if not result.is_error:
            retained = SupplierHandoffReport.model_validate_json(
        ...
        async with Client(create_server(self.root, allow_supplier_submissions=True)) as client:
            for variant in variants:
                with self.subTest(variant=variant):
                    ...
                    with (
                        patch("kicad_tooling.hwrepo.digikey_handoff._post") as post,
                        patch(
                            "kicad_tooling.hwrepo.supplier_handoff.read_model", wraps=read_model
                        ) as reader,
                    ):
                        result = await client.call_tool(
                            "submit_supplier_handoff",
                        ...
                        post.assert_not_called()
                        self.assertFalse(
                            any(call.args[0] == receipt for call in reader.call_args_list)
                        )
                    receipt.unlink()
    ...
    async def test_supplier_contracts_and_tampered_projection_reject(self) -> None:
        ...
        for record in (prepared, spec):
            ...
            for mutation in (
                ...
                with (
                    self.subTest(model=model, mutation=mutation),
                    self.assertRaises(ValidationError),
                ):
                    model.model_validate_json(json.dumps(raw))
        for invalid_url in (
            "https://example.invalid/short/abc1234",
            "http://www.digikey.com/short/abc1234",
            "https://www.digikey.com/short/abc1234?track=1",
        ):
            with self.subTest(invalid_url=invalid_url), self.assertRaises(ValidationError):
                SupplierHandoffReport.model_validate_json(
                    json.dumps(prepared.model_dump(mode="json") | {"single_use_url": invalid_url})
                )
        ...
        write_model(
            report_path,
            order.model_copy(
                update={"plan": order.plan.model_copy(update={"lines": (changed_line,)})}
            ),
        )
        with patch("kicad_tooling.hwrepo.digikey_handoff._post") as post:
            async with Client(create_server(self.root, allow_exports=True)) as client:
