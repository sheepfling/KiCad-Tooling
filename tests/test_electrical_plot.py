...
def _measure(expression: str, unit: str, start: float, stop: float) -> SimulationMeasure:
    return SimulationMeasure(id="response", expression=expression, statistic="max", unit=unit,
                             start=start, stop=stop, minimum=-1, maximum=2)


def _transient(unit: str = "A") -> TransientAnalysis:
    return TransientAnalysis(id="startup", basis="Synthetic test circuit", deck=_DECK,
                             source_sha256=_SOURCE, model_sha256=_MODELS,
                             step_s=0.1, stop_s=1.0,
                             measures=(_measure("-i(vrail)", unit, 0, 1),))


def _frequency(expression: str = "db(v(out)/v(in))", unit: str = "dB") -> FrequencyAnalysis:
    return FrequencyAnalysis(id="response", basis="Synthetic transfer function", deck=_DECK,
                             source_sha256=_SOURCE, model_sha256=_MODELS,
                             start_hz=1, stop_hz=100,
...
class ElectricalPlotTests(unittest.TestCase):
    def test_transient_uses_indexed_waveform_not_scalar_measurement(self) -> None:
        data = WaveformData(axis_name="time", axis=(0.0, 0.5, 1.0), series=(
    ...
    def test_ac_gain_has_explicit_derived_transfer_phase(self) -> None:
        data = WaveformData(axis_name="frequency", axis=(1.0, 10.0, 100.0), series=(
            WaveformSeries(name="value0", unit="decibel", values=(0j, -3 + 0j, -6 + 0j)),
            WaveformSeries(name="v(in)", unit="voltage", values=(1 + 0j,) * 3),
            WaveformSeries(name="v(out)", unit="voltage",
                           values=(1 + 0j, 1 - 1j, 0 - 1j)),
        ))
    ...
    def test_rejects_missing_or_inconsistent_waveform_instead_of_charting_check(self) -> None:
        case = _transient()
        for data, message in (
            (WaveformData(axis_name="time", axis=(0.0, 0.5, 1.0), series=(
                WaveformSeries(name="check0", unit="notype", values=(1 + 0j,) * 3),)),
    ...
    def test_rejects_complex_decibel_trace(self) -> None:
        data = WaveformData(axis_name="frequency", axis=(1.0, 10.0, 100.0), series=(
            WaveformSeries(name="value0", unit="decibel",
                           values=(0j, 1 + 1j, 2 + 2j)),
        ))
    ...
    def test_phase_unwrap_keeps_a_continuous_frequency_response(self) -> None:
        ...
        values = tuple(complex(math.cos(math.radians(angle)),
                               math.sin(math.radians(angle))) for angle in degrees)
    ...
    def test_headless_png_and_svg_show_limits_and_derived_phase(self) -> None:
        ...
        with tempfile.TemporaryDirectory(prefix="electrical-charts-") as directory:
            ...
            for label in ("max lower limit -1 dB", "max upper limit 2 dB",
                          "Derived phase", "Frequency (Hz)"):
