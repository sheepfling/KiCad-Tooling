...
class McpPartsTests(unittest.IsolatedAsyncioTestCase):
            ...
            PartsCatalog(
                ...
                parts=(
                    PartRecord(
                        id="resistor-1k",
                        revision="A",
                        description="Protocol-test identity only",
                        part_class="resistor",
                        unit="each",
                        manufacturer="Vishay",
    ...
    async def test_parts_and_preferences_have_independent_startup_capabilities(self) -> None:
        ...
        for options, prepare_enabled, save_enabled in (
            ({}, False, False),
            ({"allow_checks": True}, False, False),
            ({"allow_edits": True}, False, True),
            ({"allow_exports": True}, True, False),
        ):
            with self.subTest(options=options):
                async with Client(create_server(self.root, **options), mode="legacy") as client:
                    ...
                    if not prepare_enabled:
                        ...
                        self.assertNotIn(
                            "output", tools["prepare_parts"].input_schema["properties"]
                        )
                        result = await client.call_tool(
                            "prepare_parts",
                            {
                                "project_id": self.project_id,
                                "view_id": "cannot-capture",
                            },
                        )
    ...
    async def test_saved_evidence_prepares_quantities_and_retains_native_failure(self) -> None:
        ...
        async with Client(create_server(self.root, allow_exports=True), mode="legacy") as client:
            with patch(
                "kicad_tooling.hwrepo.contract_coach.run_command",
                side_effect=AssertionError("native execution"),
            ):
            ...
            for mutation in ("source", "project", "netlist"):
                with self.subTest(mutation=mutation):
                    ...
                    report = self.structured(
                        await client.call_tool("prepare_parts", self.arguments(mutation))
                    )
    ...
    async def test_invalid_paths_and_saved_runner_fail_before_creating_receipts(self) -> None:
        ...
        invalid = (
            {"view_id": "../escape"},
            {"view_id": "/tmp/escape"},
            {"native_summary": "../outside.json"},
            {"native_summary": str(self.native / "summary.json")},
            {"native_summary": "catalog/parts.json"},
            {"preferences": "build/linked.json"},
            {"preferences": peer.relative_to(self.root).as_posix()},
            {"preferences": "catalog/parts.json"},
            {"preferences": "../outside.json"},
            {"runner": "local"},
            {"runner": "container"},
        )
        async with Client(create_server(self.root, allow_exports=True), mode="legacy") as client:
    ...
    async def test_alternate_preferences_are_bounded_and_run_overrides_do_not_rewrite(self) -> None:
        ...
        async with Client(create_server(self.root, allow_exports=True), mode="legacy") as client:
            ...
            report = self.structured(
                await client.call_tool("prepare_parts", self.arguments("override") | {"boards": 10})
            )
    ...
    async def test_fresh_capture_uses_only_fixed_runner_with_both_capabilities(self) -> None:
        constructors = {
            "auto": "AutoNetlistRunner",
            "local": "LocalNetlistRunner",
            "container": "ContainerNetlistRunner",
        }
        async with Client(
            create_server(self.root, allow_exports=True, allow_checks=True), mode="legacy"
        ) as client:
            for selection, constructor in constructors.items():
                with self.subTest(runner=selection):
                    ...
                    with patch(
                        f"kicad_tooling.hwrepo.mcp_parts.contract_coach.{constructor}",
                        return_value=runner,
                    ) as factory:
                        report = self.structured(
                            await client.call_tool(
                                ...
                                {
                                    "project_id": self.project_id,
                                    "view_id": f"fresh-{selection}",
                                    "runner": selection,
                                },
                            )
                        )
                        factory.assert_called_once_with(
                            *(() if selection == "container" else ("kicad-cli",))
                        )
                    self.assertEqual(report["status"], "READY_FOR_ORDER_REVIEW", report["issues"])
                    self.assertFalse(report["purchase_authorized"])
                    self.assertEqual(
                        report["selected_runner"],
                        "container" if selection == "container" else "local",
                    )
                    self.assertTrue(
                        (Path(report["receipt_dir"]) / "netlist.command.json").is_file()
                    )
    ...
    async def test_preferences_create_update_and_stale_digest_preserve_exact_readback(self) -> None:
        async with Client(create_server(self.root, allow_edits=True), mode="legacy") as client:
            ...
            nonexistent = await client.call_tool(
                "save_parts_preferences", options | {"expected_sha256": "0" * 64}
            )
            ...
            self.assertEqual(
                created["path"], self.preferences_path.relative_to(self.root).as_posix()
            )
            self.assertEqual(created["after_sha256"], created["readback_sha256"])
            self.assertFalse(created["purchase_authorized"])
            read = self.structured(
                await client.call_tool(
                    "read_project_file",
                    {
                        "project_id": self.project_id,
                        "path": "docs/purchasing.json",
                    },
                )
            )
            self.assertEqual(read["sha256"], created["after_sha256"])
            updated = self.structured(
                await client.call_tool(
                    "save_parts_preferences",
                    {
                        "project_id": self.project_id,
                        "preferences": {"boards": 6, "spare_minimum": 2},
            ...
            for expected in (None, read["sha256"]):
                rejected = await client.call_tool(
                    "save_parts_preferences", options | {"expected_sha256": expected}
                )
