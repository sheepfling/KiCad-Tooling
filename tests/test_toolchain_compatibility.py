...
def record(version: str = "10.0.5", **changes: object) -> ToolchainRecord:
    return ToolchainRecord.model_validate({
        "id": "approved", "kicad_version": version,
        "image": "example.invalid/kicad@sha256:" + "a" * 64,
        "desktop_edit_policy": "approved_for_editing",
        "installer_source": "Reviewed source", "migration_policy": "Review on a branch",
        **changes,
    })
...
class ToolchainCompatibilityTest(unittest.TestCase):
    ...
    def test_new_major_requires_explicit_author_declaration(self) -> None:
        ...
        self.assertEqual("kicad-10", require_cli_profile(
    ...
    def test_doctor_blocks_undeclared_native_profile_but_preserves_portable_checks(self) -> None:
        ...
        with (
            ...
            patch("kicad_tooling.hwrepo.doctor.preflight", return_value=TemplatePreflightReport(
                template_version="1.0.0", status="PASS", issues=(),
            )),
    ...
    def test_doctor_reports_explicit_profile_without_certifying_new_version(self) -> None:
        with (
            patch("kicad_tooling.hwrepo.doctor.toolchain", return_value=record(
                "11.0.0", cli_profile="kicad-10",
            )),
            patch("kicad_tooling.hwrepo.doctor.preflight", return_value=TemplatePreflightReport(
                template_version="1.0.0", status="PASS", issues=(),
            )),
    ...
    def test_native_report_keeps_schema_and_exact_version_checks(self) -> None:
        config = ProjectConfig.model_validate_json(json.dumps({
            "schema_version": "1", "kind": "pcb_only", "assurance_profile": "training",
            "not_for_manufacture": True, "project_id": "board",
            "component_identity": {"required": False, "part_ids": []},
            "toolchain_id": "approved", "kicad_version": "11.0.0", "cli_profile": "kicad-10",
            "image": record().image, "project": "projects/board/design/main.kicad_pro",
            "source_roots": ["projects/board/design"], "required_inputs": [],
            "validation": {"kind": "pcb_only", "expected_ignored_checks": {"erc": [], "drc": []}},
        }))
        data = {
            "$schema": "https://schemas.kicad.org/drc.v1.json", "kicad_version": "11.0.0",
    ...
    def test_unspecified_toolchain_is_selected_only_when_unambiguous(self) -> None:
        with patch("kicad_tooling.check_toolchain.settings"), patch(
            "kicad_tooling.check_toolchain.repo_path", return_value=Path("catalog.json"),
            ...
            read.return_value = ToolchainsCatalog(schema_version="1", toolchains=(
                record(), record("11.0.0", id="next", cli_profile="kicad-10"),
            ))
