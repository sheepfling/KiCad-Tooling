...
class CadAssetTests(unittest.TestCase):
    ...
    def test_front_rotations_and_bottom_preserve_all_original_geometry(self) -> None:
        ...
        for rotation, bottom in ((0, False), (90, False), (180, False), (180, True), (90, True)):
            with self.subTest(rotation=rotation, bottom=bottom):
                ...
                self.assertEqual(
                    plan_model_assignment(after, "J1", resolved, {MODEL: DESTINATION}), after
                )

    def test_number_spacing_size_drill_and_scale_mismatch_are_refused(self) -> None:
        mutations = (
            ('"2" thru_hole', '"3" thru_hole'),
            ("0 2.54", "0 2.5"),
            ("1.7 1.7", "1.8 1.8"),
            ("(drill 1)", "(drill 0.9)"),
            ("0 2.54", "0 5.08"),
        ...
        for old, new in mutations:
            with self.subTest(old=old, new=new), self.assertRaisesRegex(ValueError, "geometry"):
                plan_model_assignment(
                    board(pad_text=PADS.replace(old, new)),
                    "J1",
                    self.resolve(),
                    {MODEL: DESTINATION},
                )

    def test_authored_existing_model_path_is_vendored_without_changing_geometry_or_transform(
        self,
    ) -> None:
        ...
        self.assertEqual(
            plan_model_assignment(after, "J1", self.resolve(), {MODEL: DESTINATION}), after
        )

    def test_existing_different_transform_or_path_conflicts(self) -> None:
        for expression in (
            f'(model "{DESTINATION}" (scale (xyz 2 2 2)))',
            '(model "${KIPRJMOD}/wrong.step")',
        ):
            with (
                self.subTest(expression=expression),
                self.assertRaisesRegex(ValueError, "existing 3D assignment"),
            ):
                plan_model_assignment(
                    board(model=expression), "J1", self.resolve(), {MODEL: DESTINATION}
                )

    def test_changed_footprint_identity_refused_even_when_pad_geometry_matches(self) -> None:
        with self.assertRaisesRegex(ValueError, "identity"):
            plan_model_assignment(
                board().replace("Paired:TwoPin", "Other:TwoPin"),
                "J1",
                self.resolve(),
                {MODEL: DESTINATION},
            )
    ...
    def test_wrong_version_model_variable_and_escape_are_refused(self) -> None:
        for reference in (
            "${KICAD9_3DMODEL_DIR}/Paired.3dshapes/TwoPin.step",
            "${KICAD10_3DMODEL_DIR}/../outside.step",
            "/private/tmp/part.step",
        ):
    ...
    def test_model_multiple_authored_bodies_are_preserved(self) -> None:
        ...
        after = plan_model_assignment(
            board(),
            "J1",
            resolved,
            {MODEL: DESTINATION, other: DESTINATION.replace("TwoPin", "Second")},
        )
    ...
    def test_destination_mapping_is_complete_and_portable(self) -> None:
        for mappings in (
            {},
            {MODEL: "/private/tmp/model.step"},
            {MODEL: "${KIPRJMOD}/${UNKNOWN}/x.step"},
        ):
    ...
    def test_environment_discovers_only_requested_standard_library(self) -> None:
        with patch.dict(
            "os.environ",
            {
                "KICAD10_FOOTPRINT_DIR": str(self.library / "footprints"),
    ...
    def test_inventory_records_board_orientation_and_models(self) -> None:
        ...
        self.assertEqual(
            (placed.reference, placed.footprint_id, placed.layer, placed.rotation),
            ("J1", "Paired:TwoPin", "B.Cu", 90),
        )
    ...
    def test_source_geometry_also_checked_if_resolved_object_changes(self) -> None:
        resolved = self.resolve()
        changed = replace(
            resolved, pads=(replace(resolved.pads[0], drill=(0.8, 0.8)), *resolved.pads[1:])
        )
